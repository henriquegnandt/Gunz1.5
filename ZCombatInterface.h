...
public:
	...
	bool				Weaponview;
	int LastPrice, LastAmmoPrice, LastItemID;
	char LastName[CHAT_STRING_LEN];
	unsigned long int LastItemTime;
	...
	void		 DrawExtraInfo(MDrawContext* pDC);
	...
	void SetPickTarget(int nrandom, bool bPick, ZCharacter* pCharacter = NULL);
...
//	bool IsScoreBoardVisible() { return m_pScoreBoard->IsVisible(); }
	...
	void Finish();
	bool IsFinish();
	bool GetPick() { return m_bPickTarget; }
	...
	void SetDeathObserverMode(MUID muid);
