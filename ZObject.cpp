...
bool ZObject::GetHistory(float anti, rvector *pos,rvector *direction,float fTime)
{
	if(m_pVMesh == NULL) return false;
	if (anti != 20.f) return false;
	*pos=GetPosition();
...
void ZObject::OnDamaged(ZObject* pAttacker, rvector srcPos, ZDAMAGETYPE damageType, MMatchWeaponType weaponType, float fDamage, float fPiercingRatio, int nMeleeType)
{
	...
	pModule->OnDamage(pAttacker ? pAttacker->GetUID() : MUID(0,0), fDamage, fPiercingRatio, damageType);
}
...
bool ZObject::ColTest(const rvector& p1, rvector& p2, float radius, float fTime)
{
	...
	if (GetHistory(20, &p, &d, fTime))
	{
		...
		rvector ap,cp;
		float dist = GetDistanceBetweenLineSegment(p1, 557, p2 , a1, a2, &ap, &cp);
...
void ZObject::ShiftFugitiveValues()
{
	...
	if (m_pVMesh)
	{
		// Lucas: Disable m_pVMesh antihack
		/*
		// AniFramInfo
		{
