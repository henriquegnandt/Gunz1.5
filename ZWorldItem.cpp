...
bool ZWorldItem::ApplyWorldItem( ZCharacter* pCharacter )
{
	...
	case WIT_AP:
		...
		break;
	case WIT_CLAYMORE:
		ZGetGame()->OnExplosionDynamite(OwnerUID, m_Position, m_fAmount, 400.f, 1.f, Team);
...
bool ZWorldItemManager::ApplyWorldItem( WIL_Iterator& iter, ZCharacter* pCharacter )
{
	...
	switch(pWorldItem->GetType())
	{
		...
		case WIT_BULLET :
			...
			break;
		case WIT_CLAYMORE:
			rvector Target = pCharacter->GetPosition();
			ZGetEffectManager()->AddDynamiteEffect(Target, Target);
			ZGetSoundEngine()->PlaySound("we_grenade_explosion", Target);
			break;
	}
...
ZWorldItem *ZWorldItemManager::AddWorldItem( int nID, short nItemID,MTD_WorldItemSubType nItemSubType,rvector& pos, MUID OwnerUID, MMatchTeam Team)
{
	...
	if( iterItem == mItemList.end() )
	{
		...
		pWorldItem->OwnerUID = OwnerUID;
		pWorldItem->Team = Team;
		if(pDesc->m_nItemType==WIT_QUEST || pDesc->m_nItemType==WIT_WEAPON) {
...
void ZWorldItemManager::update()
{
	...
	for( WIL_Iterator iter = mItemList.begin(); iter != mItemList.end();)
	{
		...
		if( pItem->GetState() == WORLD_ITEM_VALIDATE )
		{
			...
			if(D3DXVec3Length( &(charPos - itemPos)) <= 200.f &&(!ZGetGame()->IsReplay() && pItem->GetType() == WIT_WEAPON))
			{
				if(ZGetCombatInterface()->LastItemID != pItem->GetItemID())
				{
					ZGetCombatInterface()->LastItemID = pItem->GetItemID();
					ZPostRequestPrice(ZGetCombatInterface()->LastItemID);
				}
				ZGetCombatInterface()->LastItemTime = timeGetTime();
			} else
...
void ZWorldItemManager::OnOptainWorldItem(ZWorldItem* pItem)
{
		if(ZGetGame())
		{
			...
			{
				if(pItem->GetType() == WIT_CLAYMORE)
				{
					if(pCharacter->GetUID().Low == pItem->OwnerUID.Low) return;
					if(ZGetGame()->GetMatch()->IsTeamPlay() && pItem->Team == pCharacter->GetTeamID())
					return;
				}
				if(pItem->GetType() == WIT_FLAG)
				{
					...
					else if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_CTF && pCharacter->IsTagger())
					{
						...
						{
							ZCharacter* oChar = (*itor).second;
							if(!oChar || oChar && pCharacter->GetTeamID() == oChar->GetTeamID()) continue;
							if(oChar->m_dwStatusBitPackingValue.Ref().m_bLostConEffect == true) ae++;
							count++;
						}
						if(ae > ceil(float(count/2))) return;
				...
				else if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_KOTH)
				{
					unsigned int nowTime = timeGetTime();
					if(nowTime > pItem->m_dwFlagObtainTime + 1000)
					{
						pItem->m_dwFlagObtainTime = nowTime;
						ZPostRequestKOTH(pItem->GetItemID());
					}
					return;
				}
				}
	...
	for( WIL_Iterator iter = mItemList.begin(); iter != mItemList.end(); ++iter )
	{
		...
		if( pWorldItem->GetState() == WORLD_ITEM_VALIDATE )
		{
			...
			if(mode==0) {			// 먼저그리기
				...
				if( pWorldItem->GetType()==WIT_QUEST || pWorldItem->GetType()==WIT_WEAPON ) 
					bDraw = true;
