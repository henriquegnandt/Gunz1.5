...
bool ZMyItemList::CheckTypeWithListFilter(int type, bool bEnchantItem, bool iscash)
{
	...
	else if (m_ListFilter == zequip_item_filter_CashItem && iscash) { return true; }
	return false;
}
...
void ZMyItemList::SerializeZItemList()
{
	...
	for (int i = 0; i < (int)m_ItemIndexVector.size(); i++)
	{
		...
		if (itor != m_ItemMap.end())
		{
			...
			// 장비 목록 창
			if (false == CheckTypeWithListFilter( pItemDesc->m_nSlot, pItemDesc->IsEnchantItem(),  pItemDesc->IsCashItem())) continue;
