...
protected:
	...
	FSOUND_STREAM*	m_pRStream;
	//FSOUND_STREAM*  RecPlayBack;
	int				m_iMusicChannel;
	bool			m_b16Bit;

	// callback
	static signed char F_CALLBACKAPI STREAM_END_CALLBACK(FSOUND_STREAM *stream, void *buff, int len, int param);
	//static signed char F_CALLBACKAPI _stream_callback(FSOUND_STREAM *stream, void *buff, int len, void *userdata);
...
public:
	...
	// Voice Chat
	FSOUND_SAMPLE* RecordedSound;
	void RecordSound();
	void RecieveRecordSound(char* Buffer, int Len);
	void SendStream();
	void SetRecordDriver(int i);
