...
MImplementRTTI(ZWeaponClayMore,ZMovingWeapon);

ZWeaponClayMore::ZWeaponClayMore() :ZMovingWeapon()
{
//	m_nWeaponType = 0;
	m_fDeathTime = 0;
	m_nWorldItemID = 0;
...
ZWeaponClayMore::~ZWeaponClayMore() 
{

}

void ZWeaponClayMore::Create(RMesh* pMesh,rvector &pos, rvector &velocity,ZObject* pOwner)
{
	ZWeapon::Create(pMesh);
...
void ZWeaponClayMore::UpdateFirstPos()
{
	/////////////////////////////////////// 지금은 계산해서 보내준다...
...
void ZWeaponClayMore::UpdatePost(DWORD dwPickPassFlag, float fDropDelayTime)
{
	if(m_uidOwner == ZGetGameClient()->GetPlayerUID()) {
		ZPostRequestSpawnWorldItem(ZGetGameClient()->GetPlayerUID(), m_nWorldItemID, m_Position, fDropDelayTime);
		m_PostPos = m_Position;
...
void ZWeaponClayMore::UpdateWorldItemFloorDrop(DWORD dwPickPassFlag)
{
	RBSPPICKINFO rpi;
	bool bPicked = ZGetWorld()->GetBsp()->Pick(m_Position,rvector(0,0,-1),&rpi, dwPickPassFlag );
...
void ZWeaponClayMore::UpdatePos(float fElapsedTime,DWORD dwPickPassFlag)
{
	rvector diff = m_Velocity * fElapsedTime;
...
// 땅에 닿으면 소멸~
bool ZWeaponClayMore::Update(float fElapsedTime)
{
	if(m_bDeath) {
		return false;
	}
...
void ZWeaponClayMore::Explosion()
{
	// 자신의 이펙트 그리고..
	// 주변 유닛들에게 데미지 주는 메시지 날리고..
	// ( 메시지 받은곳에서 주변 유닛들 체크하고.. 
...
//		else
		{
			...
			if ( ZGetGame()->ObjectColTest(pOwnerObject, m_Position, to, m_pSkillDesc->fColRadius, &pPickObject))
			{
				...
				if ( bCheck)
				{
					...
					ExplosionThrow( WMET_OBJECT, pPickObject,pickdir, Team);
...
void ZWeaponMagic::Explosion(WeaponMagicExplosionType type, ZObject* pVictim,rvector& vDir, MMatchTeam t)
{
...
#define MAGIC_KNOCKBACK_CONST   .3f	
	...
	if (!m_pSkillDesc->IsAreaTarget())
	{
		...
		{
			ZGetGame()->OnExplosionMagicNonSplash(this, m_uidOwner, pVictim->GetUID(), v, m_pSkillDesc->fModKnockback, t);
		}
	}
	else
	{
		ZGetGame()->OnExplosionMagic(this,m_uidOwner,v,MAGIC_MINIMUM_DAMAGE,m_pSkillDesc->fModKnockback, t, true);
	}
	...
	if(type == WMET_OBJECT) {// object 에 부딪히는 경우...
		...
		*/
		...
		if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC && t != ZGetGame()->m_pMyCharacter->GetTeamID())
		{
...
void ZWeaponMagic::ExplosionThrow(WeaponMagicExplosionType type, ZObject* pVictim,rvector& vDir, MMatchTeam t)
{
	rvector v = m_Position-rvector(0,0,100.f);
...
#define MAGIC_KNOCKBACK_CONST   .3f	
	...
	{
		ZGetGame()->OnExplosionMagicThrow(this,m_uidOwner,v,MAGIC_MINIMUM_DAMAGE,m_pSkillDesc->fModKnockback, Team, true, m_Position, vDir + m_Position);
	}
