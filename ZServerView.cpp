...
void ZServerView::PingImageDraw(MDrawContext* pDC, MRECT rectBox, int nImageCount, bool bIsLive)
{
	...
	const int nMoveImage = int(230 * MGetWorkspaceWidth() / 800.f);
	pDC->Draw(rectBox.x+nMoveImage , rectBox.y-4, 25 , 16);
#ifdef  _DEBUG
	char strPing[16];
...
void ZServerView::ServerListDraw(MDrawContext* pDC, MRECT rectBox, ServerInfo* pServerInfo, bool bSelected)
{
	...
	/*	if ( pServerInfo->nNumOfUser >= pServerInfo->nCapacity)
		...
		sprintf( szText, "%s", pServerInfo->szName);
	}
	else
	{
...
bool ZServerView::AddServer( char* szName, char* szAddress, int nPort, int nType, int nNumOfUser, int nCapacity, bool IsLive, char* szAgentIP, int nRow, int nCol, int AgentUDPPort)
{
	...
	pServerNode->AgentUDPPort = AgentUDPPort;
	if(szAgentIP != NULL)
