...
void ZCustomOutput(const char* szMsg, ...)
{
		...
		if (strlen(szMsg) >= 2)
		{
			if (((szMsg[1] > 0) && (isspace(szMsg[1]))) == false)
			{
				...
				if ((ZGetMyInfo()->GetUGradeID() == MMUG_ADMIN) || 
					(ZGetMyInfo()->GetUGradeID() == MMUG_GM) ||
					(ZGetMyInfo()->GetUGradeID() == MMUG_EVENTMASTER) ||
					(ZGetMyInfo()->GetUGradeID() == MMUG_DEV) ||
					(ZGetMyInfo()->GetUGradeID() == MMUG_HEADGM) ||
					(ZGetMyInfo()->GetUGradeID() == MMUG_TRAIL))
				{
...
//					return true;			// 채팅창에 표시 안되게 한다.
	...
	switch (state)
	{
	case GUNZ_GAME://Steven: Admin Logs
		{
			...
			ZPostPeerChat(szMsg, ZGetMyInfo()->GetCharName(), nTeam, ZGetMyInfo()->GetUGradeID());
		}
		break;
	case GUNZ_LOBBY:
		{
...
/*	if ((loc == CL_CURRENT) && (state==GUNZ_GAME) && strstr(szMsg.c_str(), ZGetGame()->m_pMyCharacter->GetCharInfo()->szName))
	{
		if(ZGetGame() && ZGetGame()->m_pMyCharacter)
			{
				string Find[4] = {"{hp}", "{ap}", "{primary}", "{secondary}"};
				for(int i = 0; i <= 4; i++)
				{
						char text[20] = "";
						switch(i)
						{
						case 0:
							sprintf(text, "%d", (int)ZGetGame()->m_pMyCharacter->GetHP());
							break;
						case 1:
							sprintf(text, "%d", (int)ZGetGame()->m_pMyCharacter->GetAP());
							break;
						case 2:
							sprintf(text, "%d/%d", ZGetGame()->m_pMyCharacter->GetItems()->GetItem(MMCIP_PRIMARY)->GetBulletCurrMagazine(), ZGetGame()->m_pMyCharacter->GetItems()->GetItem(MMCIP_PRIMARY)->GetBulletSpare());
							break;
						case 3:
							sprintf(text, "%d/%d", ZGetGame()->m_pMyCharacter->GetItems()->GetItem(MMCIP_SECONDARY)->GetBulletCurrMagazine(), ZGetGame()->m_pMyCharacter->GetItems()->GetItem(MMCIP_SECONDARY)->GetBulletSpare());
							break;
					}
					replaceAll(szMsg, Find[i], text);
				}
			}
	}*/
