...
//	memcpy(pTarget, pSource, sizeof(REPLAY_STAGE_SETTING_NODE));
	...
	pTarget->bIsAntiLeading = pSource->IsAntiLead;
}
...
void ConvertStageSettingNodeForRecord(const MSTAGE_SETTING_NODE* pSource, REPLAY_STAGE_SETTING_NODE* pTarget)
{
	...
	pTarget->bTeamWinThePoint = pSource->bTeamWinThePoint;
	pTarget->bForcedEntryEnabled = pSource->bForcedEntryEnabled;
	pTarget->IsAntiLead = pSource->bIsAntiLeading;
}
...
bool ZReplayLoader::Load(const char* filename)
{
	...
	if (!LoadAntiReplay(file, 0)) return false;
	if (!LoadStageSetting(file)) return false;
	ChangeGameState();

	if(m_nVersion>=4)
		if (!LoadStageSettingEtc(file)) return false;
	if (!LoadAntiReplay(file, 1)) return false;
	if (!LoadCharInfo(file)) return false;
	if (!LoadAntiReplay(file, 2)) return false;
...
bool ZReplayLoader::LoadStageSetting(ZFile* file)
{
	...
	if(m_StageSetting.fuckyou != 76434) return false;
	return true;
}

bool ZReplayLoader::LoadAntiReplay(ZFile* file, int type)
{
	int result = 0;
	int nRead = zfread(&result, sizeof(result), 1, file);
	if(nRead==0) return false;
	switch (type) 
	{
	case 0:
	...
	case 1:
		...
		break;
	case 2:
		//if(result != 91919 && result != -1275396352)
			//return false;
		break;
	}
	return true;
...
void ConvertCharInfo_v8_tocurr(MTD_CharInfo* curr, MTD_CharInfov8* v8)
{
	memcpy(curr, v8, sizeof(MTD_CharInfov8));
	curr->nCID = 0;
}
...
void ConvertCharInfo(MTD_CharInfo* currCharInfo, void* oldCharInfo, int nVerOld)
{
	...
	case 5:
		ConvertCharInfo_v5_to_curr(currCharInfo, (MTD_CharInfo_v5*)oldCharInfo);
		 break;
	case 8:
		ConvertCharInfo_v8_tocurr(currCharInfo, (MTD_CharInfov8*)oldCharInfo);
		break;
