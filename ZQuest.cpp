...
bool ZQuest::OnPeerNPCSkillStart(MCommand* pCommand)
{
	...
	if (pActor)
	{
		ZCharacter* pChar = ZGetCharacterManager()->Find(uidTarget);
		if(pChar && ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC && pChar->GetTeamID() == pActor->GetTeam()) return true;
		...
		if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC)
			pmod->PreExcute(nSkill,uidTarget,targetPos, pActor->GetTeam());
		else
...
bool ZQuest::OnPeerNPCSkillExecute(MCommand* pCommand)
{
	...
	if (pActor)
	{
		...
		ZModule_Skills *pmod = (ZModule_Skills *)pActor->GetModule(ZMID_SKILLS);
		if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC)
			pmod->Excute(nSkill,uidTarget,targetPos, pActor->GetTeam());
		else
