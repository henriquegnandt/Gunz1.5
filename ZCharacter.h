...
{	// 수정시 기존 리플레이 로딩 고려해야합니다
	...
	unsigned long int TimeActivated;
	ZCharacterStatus() :	
							nLife(10),
							nKills(0),
							nScores(0),
	...
	void AddScores(int nAddedScores = 1) { nScores += nAddedScores; }
...
public:
	...
	*/
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	MProtectValue<DWORD>* m_pLastShotTime;
protected:
...
//	DWORD m_dwBackUpTime;
	...
	MMatchItemDesc* m_fLastShotDesc;
...
//	float GetMeleeWeaponRange();
	...
	virtual void InitStatus(bool ignoreammo=false, bool ignorehp=false);
	virtual void InitRound();
	virtual void InitItemBullet(bool onlynades=false);
	...
	ZItem* GetSelectItem() {
		if(GetItems())
			if(GetItems()->GetSelectedWeapon())
				return GetItems()->GetSelectedWeapon();
		return NULL;
	}
	void LevelUp();
	void LevelDown();
...
//	virtual void OnDamage(int damage, float fRatio = 1.0f);
	...
	virtual void OnDamagedAPlayer(ZObject* pAttacker, rvector srcPos, RMeshPartsType Parts, float dmg);
