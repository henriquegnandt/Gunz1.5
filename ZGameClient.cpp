...
//	gid = MMUG_GM;
	...
	/* Steven: Unmask*/
	if(bSpUser||				// 일반 유저
			  (strcmp( szName, ZGetMyInfo()->GetCharName()) == 0))	// 특수유저
	{
...
//	gid = MMUG_GM;
	...
	/* Steven: Unmask*/
	if(bSpUser||				// 일반 유저
		(strcmp( pObjCache->GetName(), ZGetMyInfo()->GetCharName()) == 0))	// 특수유저
	{
		if(ZGetMyInfo()->IsAdminGrade() && ZGetConfiguration()->ViewCid == true)
...
#if defined(_LOCATOR) //|| defined(_STEVEN)// -by 추교성. Locator에 접속해서 커맨드를 받으려면 m_This의 UID가 (0,0)이 아니어야 함.
	m_This = MUID(0, 1);
#endif
	#ifdef ENABLEDTS3
