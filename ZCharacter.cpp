...
//	RegisterModules();
	...
	m_pLastShotTime = new MProtectValue<DWORD>;
	m_pLastShotTime->Set_MakeCrc(0);
			...
			(ZC_STATE_UPPER_LOAD <= aniState_Upper && aniState_Upper <= ZC_STATE_UPPER_GUARD_CANCEL))
		{
			if(aniState_Upper!=ZC_STATE_UPPER_GUARD_IDLE) {
				...
				if(	(aniState_Lower != ZC_STATE_LOWER_ATTACK1_RET) &&
					...
					(ZGetConfiguration()->GetVideo()->bTrailOption))
					bDrawTracks = true;
			}
		}
	}
...
void ZCharacter::OnDraw()
{
	...
	/*if (g_bChams)
	{
		...
		UINT nOffset = 0, nStide = 0;
		LPDIRECT3DVERTEXBUFFER9 pStreamData;
		if (pDevice->GetStreamSource(0, &pStreamData, &nOffset, &nStide) == D3D_OK)
			pStreamData->Release();
...
//	무기타잎별로 하나만 등록되어 있으니까 기본공격 모션의 길이를 기준으로 한다..
	...
	switch(pSelectItemDesc->m_nWeaponType.Ref())
	{
		...
		case MWT_DOUBLE_GREAT_SWORD:	// attack1 공격 길이만큼.... man_sword_standshot_1.elu.ani
			nReturnDelay -= 299;
			break;
		default:
			_ASSERT(0);
...
//				DWORD nWeaponDelay = (DWORD)((float)pRangeDesc->m_nDelay * 0.7f);// 반으로 나눠서~
				...
				if(max_frame) {
					...
					speed = 5.3f * m_fAttack1Ratio.Ref();
				}
...
#define FRAME(x) int(float(x)/30.f*4800.f)
		...
		if(m_nWhichFootSound!=nCurrFoot && pMaterial) {	
			...
			{	
				// 왼발
				rvector pos = m_pVMesh->GetLFootPosition(!bMemEdited);
			...
			{
				rvector pos = m_pVMesh->GetRFootPosition(!bMemEdited);
...
void ZCharacter::InitItemBullet(bool onlynades)
{
...
void ZCharacter::InitStatus(bool ignoreammo, bool ignorehp)
{
	if(!ignorehp)
	InitHPAP();
	InitItemBullet(ignoreammo);
...
{
	if(m_MInitialInfo.Ref().nUGradeID == MMUG_GM ||
		m_MInitialInfo.Ref().nUGradeID == MMUG_ADMIN || m_MInitialInfo.Ref().nUGradeID == MMUG_DEV || m_MInitialInfo.Ref().nUGradeID == MMUG_HEADGM || m_MInitialInfo.Ref().nUGradeID == MMUG_TRAIL)
		return true;
	return false;
}
...
void ZCharacter::InitProperties()
{
	...
	/*static float*/ float fAddedAP = DEFAULT_CHAR_AP;			///< AP 재조정
	...
	/*static float*/ float fAddedHP = DEFAULT_CHAR_HP;			///< HP 재조정
	for (int i = 0; i < MMCIP_END; i++) {
		if (!m_Items.GetItem(MMatchCharItemParts(i))->IsEmpty()) {
	...
	if(pObjCache)
	{
	if (IsAdminGrade(pObjCache->GetUGrade()) && pObjCache->CheckFlag(MTD_PlayerFlags_AdminHide) || pObjCache->GetUGrade() == MMUG_DEV && pObjCache->CheckFlag(MTD_PlayerFlags_AdminHide) || pObjCache->GetUGrade() == MMUG_TRAIL && pObjCache->CheckFlag(MTD_PlayerFlags_AdminHide))
...
bool ZCharacter::Create(MTD_CharInfo* pCharInfo/*, MTD_CharBuffInfo* pCharBuffInfo*/)
{
	...
	/*GenerateTexture(pDevice, &Red, D3DCOLOR_XRGB(255, 000, 000));
	GenerateTexture(pDevice, &Black, D3DCOLOR_XRGB(000, 000, 000));*/
	...
	for (int i = 0; i < MMCIP_END; i++) {	///< 아이템 세팅
		...
		//ZCustomOutput("%d %d", pCharInfo->nEquipedItemDesc[i], pCharInfo->nEquipedItemCount[i]);
	}

	/*m_Items.EquipItem(MMCIP_AVATAR, 530502);
	m_Items.EquipItem(MMCIP_CHEST, 900126);*/
...
//	_ASSERT(nParts != 7);
	...
	if(m_Items.GetSelectedWeaponParts()==nParts && b == true) return;
	...
	if (
		(nParts == MMCIP_PRIMARY ||
			nParts == MMCIP_SECONDARY ||
			nParts == MMCIP_CUSTOM1 ||
			nParts == MMCIP_CUSTOM2) && bSword
		) return;
...
//*/
	// 글래디에이터일때는 총무기 사용 금지
	if (ZGetGame()->GetMatch()->IsRuleGladiator() || bSword == true)
	{
...
#define EXCELLENT_TIME	3.0f
	...
	{		
		...
		/*if ((ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_SOLO ||//KillCam
		...
		ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_REVERSE_BERSERKER ||
		ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_BOUNTY ||
		ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_GUNGAME) && !ZGetGame()->IsReplay())
		{
			ZGetCombatInterface()->GetObserver()->SetTarget(pLastAttacker->GetUID());
...
void ZCharacter::InitRound()
{
	...
	//ZGetEffectManager()->AddClanEmblem(this);
	if(GetUserGrade()==MMUG_STAR) {
		ZGetEffectManager()->AddStarEffect(this);        
	}
}
/*
static float value5=5.f;
static float value05=0.5f;
static float value15=16.f;
static float value20=20.f;
static float value30=30.f;*/
ZOBJECTHITTEST ZCharacter::HitTest( const rvector& origin, const rvector& to, float fTime, rvector *pOutPos )
{
	...
	//float fTrash = 15.f colocar no gethistory abaixo e deixar fTrash global pra checar
	if (ZObject::GetHistory(20, &footpos, &characterdir, fTime))
	{
		// 발위치 높이 보정 +5
			footpos.z+= g_fValue5;
		...
		if(IsRendered())
		{
			headpos = GetVisualMesh()->GetHeadPosition(!bMemEdited);
			headpos.z+= g_fValue5; // 머리위치 높이 보정 +5			
		}else
			headpos=footpos+rvector(0,0,180);		// 보이지않으면 서있는 걸로 추정 - 이거 좆치않음(bird)

		rvector rootpos=(footpos+headpos)*g_fValue05;
		...
		else
		{
			...
			Normalize(rootdir);
			float fDist=GetDistanceBetweenLineSegment(origin, 557, to,headpos+ g_fValue20*rootdir,rootpos- g_fValue20*rootdir,&ap,&cp);
			if(fDist<g_fValue31)		// 상체
			{
				rvector ap2cp=ap-cp;
				float fap2cpsq=D3DXVec3LengthSq(&ap2cp);
				float fdiff=sqrtf(g_fValue31*g_fValue31-fap2cpsq);
			...
			{
				float fDist=GetDistanceBetweenLineSegment(origin, 557, to,rootpos- g_fValue20*rootdir,footpos,&ap,&cp);
				if(fDist<g_fValue31)	// 하체
				{
					rvector ap2cp=ap-cp;
					float fap2cpsq=D3DXVec3LengthSq(&ap2cp);
					float fdiff=sqrtf(g_fValue31*g_fValue31-fap2cpsq);
...
void ZCharacter::OnDamagedAPlayer(ZObject* pAttacker, rvector srcPos, RMeshPartsType Parts, float dmg)
{
    if (pAttacker != NULL)
    {
        if (((ZCharacter*)pAttacker)->m_dwStatusBitPackingValue.Ref().m_bLostConEffect == false && pAttacker == ZGetGame()->m_pMyCharacter && this != pAttacker && !ZGetGame()->m_pMyCharacter->IsDie())
        {
			if(!ZGetGame()->GetMatch()->IsTeamPlay() || ZGetGame()->GetMatch()->IsTeamPlay() && ZGetGame()->GetMatch()->GetTeamKillEnabled() || !IsTeam(((ZCharacter*)pAttacker)))
			{
				...
				//ZPOSTANTILEAD(MCommandParameterBlob(pBlobArray, MGetBlobArraySize(pBlobArray)), GetUID());
			}
		}
    }
}
...
void ZCharacter::OnDamaged(ZObject* pAttacker, rvector srcPos, ZDAMAGETYPE damageType, MMatchWeaponType weaponType, float fDamage, float fPiercingRatio, int nMeleeType)
{
	/*if (bEventReduce)
	{
		...
		if (weaponType == MWT_SHOTGUN && fDamage >= 6.0f)
		{
		//	fDamage = 2.0f;
			fDamage = fDamage / 5; //ReduceNumber
		}
	}*/
	//if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC && pAttacker && pAttacker->IsNPC() && ((ZActor*)pAttacker)->GetTeam() == GetTeamID()) return;
	if (m_bInitialized==false) 
    ...
    bool bReturnValue = ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2 || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_CTF;
	...
	if(!bCanAttack)
		PROTECT_DEBUG_REGISTER(!bDebugRegister)
			return;
		//erindbg: implement anti-lead self-damage inflicted here at some point... if we want it to count that is. 
...
#ifdef _DAMAGE	// 내부빌드에서 대미지 정보 출력
	char szDamagePrint[256];
	dmgcount++;
	sprintf(szDamagePrint, "%s Got Damaged: [%2.1f], MY: HP[%2.1f] AP[%2.1f] - Count: %d", GetUserName(), fDamage, GetHP(), GetAP(), dmgcount);
