...
/*			if ((pMyCharacter) && (pMyCharacter->IsDie()))	//// 실서비스에서 스폰안되는 버그유발. 원인불명(_PUBLISH누락) 영구봉쇄.
	...
	case MWM_ACTIONRELEASED:
		{
			switch(pEvent->nKey){	
			case ZACTION_VOICERECORD:
			if(ZGetConfiguration()->GetEtc()->EnableVoice == true && ZGetConfiguration()->GetEtc()->EnablePPT == true) ZGetGameClient()->TsPTT(false);
				break;
			/*case ZACTION_VOICERECORD:
				ZGetSoundFMod()->SendStream();
				break;*/
			case ZACTION_USE_WEAPON:
			{	
				if(!ZGetGame()->m_pMyCharacter->GetItems()->GetSelectedWeapon() || !ZGetGame()->m_pMyCharacter->GetItems() || !ZGetGame()->m_pMyCharacter) return true;
				MMatchItemDesc* PItem = ZGetGame()->m_pMyCharacter->GetItems()->GetSelectedWeapon()->GetDesc();
				if(!PItem) return true;
				if(ZGetGame()->m_pMyCharacter->GetItems()->GetSelectedWeaponParts() != MMCIP_PRIMARY && ZGetGame()->m_pMyCharacter->GetItems()->GetSelectedWeaponParts() !=MMCIP_SECONDARY) return true;
				if(!PItem && PItem->m_nWeaponType.Ref() != MWT_PISTOL && PItem->m_nWeaponType.Ref() != MWT_PISTOLx2 && PItem->m_nWeaponType.Ref() != MWT_PISTOLx2 && PItem->m_nWeaponType.Ref() != MWT_REVOLVER && PItem->m_nWeaponType.Ref() != MWT_REVOLVERx2  && PItem->m_nWeaponType.Ref() != MWT_SHOTGUN  && PItem->m_nWeaponType.Ref() != MWT_DOUBLE_SHOTGUN  && PItem->m_nWeaponType.Ref() != MWT_SAWED_SHOTGUN && PItem->m_nWeaponType.Ref() != MWT_ROCKET && PItem->m_nWeaponType.Ref() != MWT_SNIFER) return true;
				unsigned long int nTime = timeGetTime();
				unsigned long int TimeDiff = (nTime - ZGetGame()->LastShotTime);
				if(TimeDiff <= 100)
				{
						ZGetGame()->nShotFlag++;	
						ZGetGame()->LastWarningTime = nTime;
				}
				if(ZGetGame()->nShotFlag >= 35)
				{
					char szText[340];
					sprintf(szText, "SB Delay Alert");
					ZPOSTCMD1(MC_STAFF_HELP, MCommandParameterString(szText));
					ZGetGame()->nShotFlag = 0;
				}
				else
				{
					if((nTime - ZGetGame()->LastWarningTime) >= 120000)
					{
						ZGetGame()->nShotFlag = 0;
					}
				}
				ZGetGame()->LastShotTime = nTime;
				return true;
			}
	...
	case MWM_ACTIONPRESSED:
		if ( !ZGetGame()->IsReservedSuicide())		// 자살 예정인 경우 대쉬를 할수없게 막는다
		{
			switch(pEvent->nKey){
			case ZACTION_VOICERECORD:
				if(ZGetConfiguration()->GetEtc()->EnableVoice == true && ZGetConfiguration()->GetEtc()->EnablePPT == true) ZGetGameClient()->TsPTT(true);
				break;
			/*case ZACTION_VOICERECORD:
					ZGetSoundFMod()->RecordSound();
				break;*/
			case ZACTION_FORWARD:
			case ZACTION_BACK:
			case ZACTION_LEFT:
			...
			case ZACTION_ITEM2:
				...
				case ZACTION_HPAP:
				{
					...
					if ((nNowTime-ZGetGame()->st_nLastTime[0]) < DELAY) break;
					ZGetGame()->st_nLastTime[0] = nNowTime;
					char szTemp[256];
					if(ZGetGame()->GetMatch()->IsTeamPlay() && (int)ZGetGame()->m_pMyCharacter->GetHP() >= 0) 
					{
						char szTeamName[100];
						sprintf(szTeamName, "(Team)%s", ZGetGame()->m_pMyCharacter->GetCharInfo()->szName);
						sprintf(szTemp, "^3HP: %d ^1/ ^2AP: %d ^0/ ^1Damage Given: %d",(int)ZGetGame()->m_pMyCharacter->GetHP(),(int)ZGetGame()->m_pMyCharacter->GetAP(), /*ZGetGame()->m_pMyCharacter->GetStatus().Ref().nRoundTakenDamage*/ ZGetGame()->m_pMyCharacter->GetStatus().Ref().nRoundGivenDamage);
						ZPostPeerChat(szTemp, szTeamName, (int)ZGetGame()->m_pMyCharacter->GetTeamID());
					}
					else if ((int)ZGetGame()->m_pMyCharacter->GetHP() >= 0)
					{
						sprintf(szTemp, "^3HP: %d ^1/ ^2AP: %d ^0/ ^1Damage Given: %d",(int)ZGetGame()->m_pMyCharacter->GetHP(),(int)ZGetGame()->m_pMyCharacter->GetAP(), /*ZGetGame()->m_pMyCharacter->GetStatus().Ref().nRoundTakenDamage*/ ZGetGame()->m_pMyCharacter->GetStatus().Ref().nRoundGivenDamage);
						ZPostPeerChat(szTemp, ZGetGame()->m_pMyCharacter->GetCharInfo()->szName, 0);
					}
					return true;
			}
			...
			case ZACTION_USE_WEAPON2:
				{
					ZItem* pSelectedItem = ZGetGame()->m_pMyCharacter->GetItems()->GetSelectedWeapon();
					if (pSelectedItem)
					{
						if(pSelectedItem->GetDesc()) 
						{
							switch (pSelectedItem->GetDesc()->m_nWeaponType.Ref())
							{
								case MWT_SNIFER:
								{
									float zoom = pSelectedItem->GetDesc()->m_nSnipeZoom.Ref();
									if(zoom >= 25) return false;
									ZGetGame()->m_pMyCharacter->OnGadget_Snifer(zoom);
								}
								return true;
							}
			...
			case ZACTION_PREVIOUS:
				{
					unsigned long int nNowTime = timeGetTime();
					if ((nNowTime-ZGetGame()->st_nLastTime[1]) < DELAY) break;
					ZGetGame()->st_nLastTime[1] = nNowTime;
					ZApplication::GetSoundEngine()->Control(Prv);
			...
			case ZACTION_NEXT:
				{
					...
					if ((nNowTime-ZGetGame()->st_nLastTime[1]) < DELAY) break;
					ZGetGame()->st_nLastTime[1] = nNowTime;
					ZApplication::GetSoundEngine()->Control(Next);
					ZApplication::GetSoundEngine()->PlayMusic(false);
					return true;
				}
			case ZACTION_PAUSE:
				{
					ZApplication::GetSoundEngine()->Control(Pause);
					return true;
				}
			} // switch
		}
	...
	case MWM_KEYDOWN:
		{
			...
			case 'T' :
				{
					MMatchObjCache* pObjCache1 = ZGetGameClient()->FindObjCache(ZGetMyUID());
					if((IsAdminGrade(pObjCache1->GetUGrade()) && ZGetGameClient()->IsLadderGame() == false || IsAdminGrade(pObjCache1->GetUGrade()) && pObjCache1->CheckFlag(MTD_PlayerFlags_AdminHide) || ZGetGame()->m_pMyCharacter->GetTeamID()==MMT_SPECTATOR) &&
