...
ZZombie::ZZombie() : m_bLoaded(false), m_bCreatedOnce(false)
{
...
void ZZombie::OnGameCreate()
{
	...
	// 때문에 ZZombie::OnDestroyOnce()는 명시적으로 호출하지 않음.
	OnCreateOnce();
	Create();

}

void ZZombie::OnGameDestroy()
{
	if (!ZGetGameTypeManager()->IsQuestDerived(ZGetGameClient()->GetMatchStageSetting()->GetGameType())) return;
...
void ZZombie::OnGameUpdate(float fElapsed)
{
	if (!ZGetGameTypeManager()->IsQuestDerived(ZGetGameClient()->GetMatchStageSetting()->GetGameType())) return;

	UpdateNavMeshWeight(fElapsed);
	m_fElapsedHealthUpdateTime += fElapsed;

	if (20.0f < m_fElapsedHealthUpdateTime)
	{
		...
		for (ZCharacterManager::iterator itor = ZGetGame()->m_CharacterManager.begin();	itor != ZGetGame()->m_CharacterManager.end(); ++itor)
		{
			...
			if(!pCharacter->IsDie())
			{
				float fAP = max(0.0f, pCharacter->GetAP() + 5);
				pCharacter->SetAP(fAP);
				float fHP = max(1.0f, pCharacter->GetHP() + 10);
	...
	void ZZombie::OnGameDestroy() {} 
	void ZZombie::OnGameUpdate(float fElapsed) {} 
	bool ZZombie::OnCreate(){return true;}
	void ZZombie::OnDestroy() {}
#endif

void ZZombie::UpdateNavMeshWeight(float fDelta)
{
	// NavMesh 가중치 업데이트
...
bool ZZombie::OnCommand(MCommand* pCommand)
{
...
bool ZZombie::OnGameCommand(MCommand* pCommand)
{
	switch (pCommand->GetID())
	{
		...
		HANDLE_COMMAND(MC_ZOMBIE_NEW_ROUND							, OnNewRound);
		HANDLE_COMMAND(MC_ZOMBIE_NPCDROP							, OnPeerDrop);
#ifdef _QUEST_ITEM
...
bool ZZombie::OnZombieKill(MCommand* pCommand)
{
...
bool ZZombie::OnNPCSpawn(MCommand* pCommand)
{
	if (ZGetGame() == NULL) return false;
	...
	pCommand->GetParameter(&InstaKill,		4, MPT_BOOL);
...
//	if (ZIsLaunchDevelop())
	...
	if (pNewActor)
	{
		...
		if(InstaKill)
		{
			ZModule_HPAP *pModule = (ZModule_HPAP*)pNewActor->GetModule(ZMID_HPAP);
			if(pModule)
			{
				pModule->SetHP(1);
				pModule->SetAP(1);
...
bool ZZombie::OnNPCDead(MCommand* pCommand)
{
...
bool ZZombie::OnEntrustNPCControl(MCommand* pCommand)
{
	MUID uidChar, uidNPC;
	pCommand->GetParameter(&uidChar,	0, MPT_UID);
	pCommand->GetParameter(&uidNPC,		1, MPT_UID);
...
bool ZZombie::OnPeerNPCBasicInfo(MCommand* pCommand)
{
...
bool ZZombie::OnPeerNPCHPInfo(MCommand* pCommand)
{

	return true;
}

bool ZZombie::OnPeerNPCBossHpAp(MCommand* pCommand)
{
	MUID uidBoss;
	float fHp, fAp;
...
bool ZZombie::OnPrePeerNPCAttackMelee(MCommand* pCommand)	// 실제로 처리하는건 한타이밍 늦다
{
	// TODO: 이때 애니메이션을 시작
	return true;
}

bool ZZombie::OnPeerNPCAttackMelee(MCommand* pCommand)
{
	MUID uidOwner;
...
bool ZZombie::OnPeerNPCAttackRange(MCommand* pCommand)
{
	MUID uidOwner;
	pCommand->GetParameter(&uidOwner,	0, MPT_UID);
...
bool ZZombie::OnRefreshPlayerStatus(MCommand* pCommand)
{
	// 운영자 hide는 제외
	...
	for(ZCharacterManager::iterator i = ZGetCharacterManager()->begin();i!=ZGetCharacterManager()->end();i++)
	{
		...
		if (!pCharacter->IsAdminHide() && pCharacter->IsDie())	pCharacter->InitStatus(false, false); else if (!pCharacter->IsAdminHide()) pCharacter->InitStatus(true, true);
	}
...
bool ZZombie::OnClearAllNPC(MCommand* pCommand)
{
...
bool ZZombie::OnQuestRoundStart(MCommand* pCommand)
{
...
bool ZZombie::OnQuestPlayerDead(MCommand* pCommand)
{
	MUID uidVictim;
...
bool ZZombie::OnQuestGameInfo(MCommand* pCommand)
{
	MCommandParameter* pParam = pCommand->GetParameter(0);
	if(pParam->GetType()!=MPT_BLOB) return false;
...
bool ZZombie::OnQuestCombatState(MCommand* pCommand)
{
	char nState;
...
bool ZZombie::OnMovetoPortal(MCommand* pCommand)
{
...
bool ZZombie::OnReadyToNewSector(MCommand* pCommand)
{
	MUID uidPlayer;
	pCommand->GetParameter(&uidPlayer,	0, MPT_UID);
...
bool ZZombie::OnSectorStart(MCommand* pCommand)
{
	char nSectorIndex;
...
bool ZZombie::OnQuestCompleted(MCommand* pCommand)
{
...
bool ZZombie::OnQuestFailed(MCommand* pCommand)
{
	mlog("Quest Failed\n");
...
bool ZZombie::OnObtainQuestItem(MCommand* pCommand)
{
...
bool ZZombie::OnObtainZItem(MCommand* pCommand)
{
...
void ZZombie::LoadNPCSounds()
{
	if (!m_GameInfo.IsInited())
	{
		mlog("ZZombie::LoadNPCSounds - not inialized Quest Game Info\n");
		return;
	}
...
#ifdef _QUEST_ITEM
bool ZZombie::OnRewardQuest( MCommand* pCmd )
{
	if( 0 == pCmd )
		return false;
...
void ZZombie::GetMyObtainQuestItemList( int nRewardXP, int nRewardBP, void* pMyObtainQuestItemListBlob, void* pMyObtainZItemListBlob )
{
...
bool ZZombie::OnNewMonsterInfo( MCommand* pCmd )
{
	char nMonIndex;
...
bool ZZombie::OnPeerNPCSkillStart(MCommand* pCommand)
{
...
bool ZZombie::OnPeerNPCSkillExecute(MCommand* pCommand)
{
	MUID uidOwner,uidTarget;
	int nSkill;
	rvector targetPos;
...
bool ZZombie::OnSetMonsterBibleInfo( MCommand* pCmd )
{
	if( 0 == pCmd )
		return false;
...
bool ZZombie::OnPeerNPCDead(MCommand* pCommand)
{
...
bool ZZombie::OnSectorBonus(MCommand* pCommand)
{
	MUID uidPlayer;
...
bool ZZombie::OnQuestPing(MCommand* pCommand)
{
...
bool ZZombie::OnNewRound(MCommand* pCommand)
{
	int round;
	pCommand->GetParameter(&round,	0, MPT_INT);
	ZGetScreenEffectManager()->AddRoundStart(round);
	ZGetGame()->GetMatch()->SetRound(round);
	return true;
}
bool ZZombie::OnPeerDrop(MCommand* pCommand)
{
	int type;
	...
	{
	case 0:
		ZGetGame()->m_pMyCharacter->InitItemBullet(true);
		break;
	case 1:
		ZGetGame()->m_pMyCharacter->InitItemBullet(false);
		break;
	case 2:
		ZGetObjectManager()->ClearNPC();
		break;
	case 3:
		...
		for(ZNPCObjectMap::iterator i = pNPCObjectMap->begin();i!=pNPCObjectMap->end();i++)
		{
			...
			if (pNPCObject)
			{
				ZModule_HPAP *pModule = (ZModule_HPAP*)pNPCObject->GetModule(ZMID_HPAP);
				if(pModule)
				{
					pModule->SetHP(1);
					pModule->SetAP(1);
