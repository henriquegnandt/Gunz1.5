	...
	*/
	...
	mlog("Step: %d\n", Step++);
		...
		if((error = ts3client_getDefaultPlayBackMode(&mode)) != ERROR_ok) {
			mlog("Error getting default playback mode: %d\n", error);
		}
	mlog("Step: %d\n", Step++);
		if((error = ts3client_getDefaultCaptureMode(&mode2)) != ERROR_ok) {
			mlog("Error getting default Capture mode: %d\n", error);
		}
	mlog("Step: %d\n", Step++);
...
#endif
	mlog("Step: %d\n", Step++);
	MComboBox *pMics = (MComboBox*)pResource->FindWidget("VoiceCombo");
	if(pMics)
	{
		#ifdef ENABLEDTS3
		pMics->RemoveAll();
		for(int i=0; device2[i] != NULL; ++i) 
		{
			char *name = device2[i][0];
			pMics->Add(name);
		}
#endif
	}
	...
	pMics = (MComboBox*)pResource->FindWidget("PlayBkCombo");
	if(pMics)
	{
		#ifdef ENABLEDTS3
		pMics->RemoveAll();
	...
	pWidget = (MSlider*)pResource->FindWidget("TSVolumeSlider");
	if(pWidget)
	{
		#ifdef ENABLEDTS3
		pWidget->SetMinMax(0, 10);
		pWidget->SetValue(ZGetConfiguration()->GetEtc()->VoiceVol);
		char vol[100];
		sprintf(vol, "%.2f", ZGetConfiguration()->GetEtc()->VoiceVol);
					char* mode;
			ts3client_getDefaultCaptureMode(&mode);
			char*** device;
			ts3client_getCaptureDeviceList(mode,&device);
			unsigned int error;
			if (device[ZGetConfiguration()->GetEtc()->VoiceDefaultDriver] != NULL)
		if((error = ts3client_setPlaybackConfigValue(ZGetGameClient()->scHandlerID, "volume_modifier", vol)) != ERROR_ok) {
			printf("Error setting playback config value: %d\n", error);
		}
#endif
	}
	mlog("Step: %d\n", Step++);
	pWidget = (MSlider*)pResource->FindWidget("VoiceDetectionSlider");
	if(pWidget)
	{
		#ifdef ENABLEDTS3
		pWidget->SetMinMax(-100, 100);
		pWidget->SetValue(ZGetConfiguration()->GetEtc()->VoiceDetVol);
		int val = 0;
		if(ZGetConfiguration()->GetEtc()->VoiceDetVol <= 32 && ZGetConfiguration()->GetEtc()->VoiceDetVol != 32)
			val -= ZGetConfiguration()->GetEtc()->VoiceDetVol;
		else if (ZGetConfiguration()->GetEtc()->VoiceDetVol >=32)
			val += ZGetConfiguration()->GetEtc()->VoiceDetVol;
		char vol[100];
		sprintf(vol, "%d", val);
					char* mode;
			...
			ts3client_getCaptureDeviceList(mode,&device);
			if (device[ZGetConfiguration()->GetEtc()->VoiceDefaultDriver] != NULL)
		ts3client_setPreProcessorConfigValue(ZGetGameClient()->scHandlerID, "voiceactivation_level", vol);
#endif
	}
	mlog("Step: %d\n", Step++);
	...
	//	Button
	{
		...
		MButton* pBtnTrail = (MButton*)pResource->FindWidget("TrailOption");
		if ( pBtnTrail )
		{
			pBtnTrail->SetCheck(ZGetConfiguration()->GetVideo()->bTrailOption);
		}
		MButton* pBtnFPS = (MButton*)pResource->FindWidget("FPSOption");
		if ( pBtnFPS )
		{
			pBtnFPS->SetCheck(ZGetConfiguration()->GetVideo()->bFpsCounter);
		}	
		
		MSlider * pCharZoom = (MSlider*)pResource->FindWidget("CharZoom");
		if(pCharZoom)
		{
			pCharZoom->SetMinMax(0, 580);
			pCharZoom->SetValue(ZGetConfiguration()->GetVideo()->Zoom);
			ZGetCamera()->m_fDist = ZGetConfiguration()->GetVideo()->Zoom;
		}
		...
		MButton* pBtn3dsound = (MButton*)pResource->FindWidget("3dsound");
		if ( pBtn3dsound )
		{
			pBtn3dsound->SetCheck(ZGetConfiguration()->GetEtc()->b3dsound);
		}
	...
	mlog("Step: %d\n", Step++);
		...
		//Steven: Play Custom Music
		pWidget = (MButton*)pResource->FindWidget("CustomMusic");
		if(pWidget)
		{
			pWidget->SetCheck(ZGetConfiguration()->GetAudio()->bCustomMusic);
		}
		pWidget = (MButton*)pResource->FindWidget("RandomMusic");
		if(pWidget)
		{
			pWidget->SetCheck(ZGetConfiguration()->GetAudio()->bRandom);
		}
		pWidget = (MButton*)pResource->FindWidget("ShowNotifications");
		if(pWidget)
		{
			pWidget->SetCheck(ZGetConfiguration()->GetAudio()->bNotfications);
		}
		...
		pWidget = (MButton*)pResource->FindWidget("RemoveCustomsFromMouseScroll");
		if(pWidget)
		{
			pWidget->SetCheck(ZGetConfiguration()->GetRemoveCustomsFromMouseScroll());
		}
		...
		pWidget = (MButton*)pResource->FindWidget("BulletEffect");
		if(pWidget)
		{
			pWidget->SetCheck(ZGetConfiguration()->GetVideo()->bBulletEffect);
		}
	}
	...
	{	
		...
		MButton* pBtnEnableVoice= (MButton*)pResource->FindWidget("EnableVoice");
		if ( pBtnEnableVoice )
		{
			pBtnEnableVoice->SetCheck(ZGetConfiguration()->GetEtc()->EnableVoice);
		}
	mlog("Step: %d\n", Step++);

		MButton* pBtnEnableECHO = (MButton*)pResource->FindWidget("EnableECHO");
		if ( pBtnEnableECHO )
		{	
			#ifdef ENABLEDTS3
			char* mode;
			...
			ts3client_getCaptureDeviceList(mode,&device);
			if (device[ZGetConfiguration()->GetEtc()->VoiceDefaultDriver] != NULL)
			ts3client_setPreProcessorConfigValue(ZGetGameClient()->scHandlerID, "echo_canceling", (ZGetConfiguration()->GetEtc()->EnableECHO ? "true" : "false"));
			pBtnEnableECHO->SetCheck(ZGetConfiguration()->GetEtc()->EnableECHO);
#endif
		}
	mlog("Step: %d\n", Step++);
		MButton* pBtnEnablePPT = (MButton*)pResource->FindWidget("EnablePPT");
		if ( pBtnEnablePPT )
		{
			#ifdef ENABLEDTS3
			//push to talk
			pBtnEnablePPT->SetCheck(ZGetConfiguration()->GetEtc()->EnablePPT);
#endif
		}
		MButton* pBtnEnableDVI = (MButton*)pResource->FindWidget("DisplayVoiceIcon");
		if ( pBtnEnableDVI )
		{
			pBtnEnableDVI->SetCheck(ZGetConfiguration()->GetEtc()->EnableDVI);
		}
		MButton* pBtnEnableSUPP = (MButton*)pResource->FindWidget("EnableSUPP");
		if ( pBtnEnableSUPP )
		{		
			#ifdef ENABLEDTS3
			char* mode;
			...
			ts3client_getCaptureDeviceList(mode,&device);
			if (device[ZGetConfiguration()->GetEtc()->VoiceDefaultDriver] != NULL)
			ts3client_setPreProcessorConfigValue(ZGetGameClient()->scHandlerID, "denoise", (ZGetConfiguration()->GetEtc()->EnableSUPP ? "true" : "false"));
			pBtnEnableSUPP->SetCheck(ZGetConfiguration()->GetEtc()->EnableSUPP);
#endif
		}
	mlog("Step: %d\n", Step++);
		MComboBox* pcmbo = (MComboBox*)pResource->FindWidget("VoiceCombo");
		if(pcmbo)	
		{
			#ifdef ENABLEDTS3
			if(ZGetGameClient() && ZGetConfiguration()->GetEtc()->VoiceDefaultDriver != -1) 
			{
			pcmbo->SetSelIndex(ZGetConfiguration()->GetEtc()->VoiceDefaultDriver);
			char* mode;
			ts3client_closeCaptureDevice(ZGetGameClient()->scHandlerID);
			ts3client_getDefaultCaptureMode(&mode);
			char*** device;
			ts3client_getCaptureDeviceList(mode,&device);
			if (device[ZGetConfiguration()->GetEtc()->VoiceDefaultDriver] != NULL)
				ts3client_openCaptureDevice(ZGetGameClient()->scHandlerID, mode, device[ZGetConfiguration()->GetEtc()->VoiceDefaultDriver][1]);
			}
#endif
		}
	mlog("Step: %d\n", Step++);
		MComboBox* pbcmbo = (MComboBox*)pResource->FindWidget("PlayBkCombo");
		if(pbcmbo)	
		{
			#ifdef ENABLEDTS3
			if(ZGetGameClient() && ZGetConfiguration()->GetEtc()->PBDefaultDriver != -1) 
			{
			pbcmbo->SetSelIndex(ZGetConfiguration()->GetEtc()->PBDefaultDriver);
			ts3client_closePlaybackDevice(ZGetGameClient()->scHandlerID);
			char* mode1;
			ts3client_getDefaultPlayBackMode(&mode1);
			char*** device1;
			ts3client_getPlaybackDeviceList(mode1,&device1);
			if (device1[ZGetConfiguration()->GetEtc()->PBDefaultDriver] != NULL)
				ts3client_openPlaybackDevice(ZGetGameClient()->scHandlerID, mode, device1[ZGetConfiguration()->GetEtc()->PBDefaultDriver][1]);
			}
#endif
		}		
	mlog("Step: %d\n", Step++);
			
		MButton* pBtnDmg = (MButton*)pResource->FindWidget("DAMAGEOption");
		if ( pBtnDmg )
		{
			pBtnDmg->SetCheck(ZGetConfiguration()->GetEtc()->bDamageCounter);
		}
		MButton* pBtnHPAP = (MButton*)pResource->FindWidget("HPAPOption");
		if ( pBtnHPAP )
		{
			pBtnHPAP->SetCheck(ZGetConfiguration()->GetEtc()->bHpAp);
		}		
		MButton* pBtncHPAP = (MButton*)pResource->FindWidget("CLANHPAPOption");
		if ( pBtncHPAP )
		{
			pBtncHPAP->SetCheck(ZGetConfiguration()->GetEtc()->bcHpAp);
		}
		MButton* pBtnNozzleFlare = (MButton*)pResource->FindWidget("NozzleFlareOption");
		if ( pBtnNozzleFlare )
		{
			pBtnNozzleFlare->SetCheck(ZGetConfiguration()->GetEtc()->bNozzleFlare);
		}
		MButton* pBtnBulletProjectiles = (MButton*)pResource->FindWidget("BulletProjectilesOption");
		if ( pBtnBulletProjectiles )
		{
			pBtnBulletProjectiles->SetCheck(ZGetConfiguration()->GetEtc()->bBulletProjectiles);
		}
		MButton* pBtnShotgunSparks = (MButton*)pResource->FindWidget("ShotgunSparksOption");
		if ( pBtnShotgunSparks )
		{
			pBtnShotgunSparks->SetCheck(ZGetConfiguration()->GetEtc()->bShotgunSparks);
		}
		...
		if(pComboBox)
		{
			...
			for(int i = 0; i < 10; i++)
			{
				...
				{
					sprintf(szCustomFile, "%s%s%d%s", PATH_CUSTOM_CROSSHAIR, FN_CROSSHAIR_HEADER, (i-1), FN_CROSSHAIR_TAILER);
					char text[50];
					sprintf(text, "Custom %d", (i-1));
					if (IsExist(szCustomFile)) pComboBox->Add(text);
				}
			}
...
//		ZGetInput()->UnregisterActionKey(i);
	...
	*/

	{
		...
		pWidget = (MComboBox*)pResource->FindWidget("VoiceCombo");
		if(pWidget && ZGetConfiguration()->GetEtc()->EnableVoice == true)	
		{
			#ifdef ENABLEDTS3
			ZGetConfiguration()->GetEtc()->VoiceDefaultDriver = pWidget->GetSelIndex();
			if(ZGetConfiguration()->GetEtc()->VoiceDefaultDriver != -1)
			{
				ts3client_closeCaptureDevice(ZGetGameClient()->scHandlerID);
				char* mode;
				if(ts3client_getDefaultCaptureMode(&mode) == ERROR_ok) 
				{
					char*** device;
					if(ts3client_getCaptureDeviceList(mode, &device) == ERROR_ok && device[ZGetConfiguration()->GetEtc()->VoiceDefaultDriver][1] != NULL) 
					{
						if(ts3client_openCaptureDevice(ZGetGameClient()->scHandlerID, mode, device[ZGetConfiguration()->GetEtc()->VoiceDefaultDriver][1]) != ERROR_ok) 
							mlog("[Voice]Error Opening Capture Device\n");
						ts3client_freeMemory(device);
					} 
					else mlog("[Voice]Error getting playback device list\n");
				} 
				else mlog("[Voice]Error getting default playback mode\n");
			}
#endif
		}
		pWidget = (MComboBox*)pResource->FindWidget("PlayBkCombo");
		if(pWidget && ZGetConfiguration()->GetEtc()->EnableVoice == true)
		{
			#ifdef ENABLEDTS3
			ZGetConfiguration()->GetEtc()->PBDefaultDriver = pWidget->GetSelIndex();
			if(ZGetConfiguration()->GetEtc()->PBDefaultDriver != -1)
			{
				ts3client_closePlaybackDevice(ZGetGameClient()->scHandlerID);
				char* mode;
				if(ts3client_getDefaultPlayBackMode(&mode) == ERROR_ok) 
				{
					char*** device;
					if(ts3client_getPlaybackDeviceList(mode, &device) == ERROR_ok && device[ZGetConfiguration()->GetEtc()->PBDefaultDriver][1] != NULL) 
					{
						if(ts3client_openPlaybackDevice(ZGetGameClient()->scHandlerID, mode, device[ZGetConfiguration()->GetEtc()->PBDefaultDriver][1]) != ERROR_ok) 
							mlog("[PlayBack]Error Opening Capture Device\n");
						ts3client_freeMemory(device);
					} 
					else mlog("[PlayBack]Error getting playback device list\n");
				} 
				else mlog("[PlayBack]Error getting default playback mode\n");
			}
#endif
		} 
		MSlider* psWidget = (MSlider*)pResource->FindWidget("TSVolumeSlider");
		if(psWidget && ZGetConfiguration()->GetEtc()->EnableVoice == true)
		{
			#ifdef ENABLEDTS3
			ZGetConfiguration()->GetEtc()->VoiceVol = (float) ((MSlider*)psWidget)->GetValue();
			unsigned int error;
			char vol[100];
			sprintf(vol, "%.2f", ZGetConfiguration()->GetEtc()->VoiceVol);
			char* mode;
		...
		psWidget = (MSlider*)pResource->FindWidget("VoiceDetectionSlider");
		if(psWidget && ZGetConfiguration()->GetEtc()->EnableVoice == true)
		{
			#ifdef ENABLEDTS3
			ZGetConfiguration()->GetEtc()->VoiceDetVol = ((MSlider*)psWidget)->GetValue();
			int val = 0;
			if(ZGetConfiguration()->GetEtc()->VoiceDetVol <= 75 && ZGetConfiguration()->GetEtc()->VoiceDetVol != 75)
				val -= ZGetConfiguration()->GetEtc()->VoiceDetVol;
			else if (ZGetConfiguration()->GetEtc()->VoiceDetVol >=75)
				val += ZGetConfiguration()->GetEtc()->VoiceDetVol;
			char vol[100];
			sprintf(vol, "%d", val);
			char* mode;
		...
		psWidget = (MSlider*)pResource->FindWidget("CharZoom");
		if(psWidget)
		{
			ZGetConfiguration()->GetVideo()->Zoom = (float)((MSlider*)psWidget)->GetValue();
			ZGetCamera()->m_fDist = ZGetConfiguration()->GetVideo()->Zoom;
		}
		pWidget = (MComboBox*)pResource->FindWidget("MapTexLevel");
	...
	}
	{
		...
		if ( pBtn3dsound )
		{
			ZGetConfiguration()->GetEtc()->b3dsound = pBtn3dsound->GetCheck();
		}
		}
		//pWidget	= (MButton*)pResource->FindWidget("Effect3D");
		//if(pWidget)
		...
		if(pWidget)
		{
			ZGetConfiguration()->GetAudio()->bCustomMusic = pWidget->GetCheck();
		}
		pWidget = (MButton*)pResource->FindWidget("RandomMusic");
		if(pWidget)
		{
			ZGetConfiguration()->GetAudio()->bRandom = pWidget->GetCheck();
		}
		pWidget = (MButton*)pResource->FindWidget("ShowNotifications");
		if(pWidget)
		{
			ZGetConfiguration()->GetAudio()->bNotfications = pWidget->GetCheck();
		}
		pWidget = (MButton*)pResource->FindWidget("InvertMouse");
		if(pWidget)
		{
		...
		if(pWidget)
		{
			ZGetConfiguration()->SetRemoveCustomsFromMouseScroll(pWidget->GetCheck());
		}
		pWidget = (MButton*)pResource->FindWidget("FullScreen");
		if(pWidget)
		{
			ZGetConfiguration()->GetVideo()->bFullScreen = pWidget->GetCheck();
		...
		if(pWidget)
		{
			ZGetConfiguration()->GetVideo()->bBulletEffect = pWidget->GetCheck();
		}

				//Trail Option
		MButton* pTrail = (MButton*)pResource->FindWidget("TrailOption");
		if(pTrail)
		{
			Z_VIDEO_TRAIL_OPTION = pTrail->GetCheck();
		}
		MButton* pFPS = (MButton*)pResource->FindWidget("FPSOption");
		if(pFPS)
		{
			Z_VIDEO_FPS_OPTION = pFPS->GetCheck();
		}
	}
	{	// Etc
		MEdit* pEdit = (MEdit*)pResource->FindWidget("NetworkPort1");
		...
		if ( pBtnEnableVoice )
		{
			ZGetConfiguration()->GetEtc()->EnableVoice = pBtnEnableVoice->GetCheck();
		}
		MButton* pBtnEnableECHO= (MButton*)pResource->FindWidget("EnableECHO");
		if ( pBtnEnableECHO && ZGetConfiguration()->GetEtc()->EnableVoice == true)
		{	
			#ifdef ENABLEDTS3
						char* mode;
			ts3client_getDefaultCaptureMode(&mode);
			...
			if (device[ZGetConfiguration()->GetEtc()->VoiceDefaultDriver] != NULL)
			ts3client_setPreProcessorConfigValue(ZGetGameClient()->scHandlerID, "echo_canceling", (ZGetConfiguration()->GetEtc()->EnableECHO ? "true" : "false"));
			ZGetConfiguration()->GetEtc()->EnableECHO = pBtnEnableECHO->GetCheck();
#endif
		}
		MButton* pBtnEnableSUPP= (MButton*)pResource->FindWidget("EnableSUPP");
		if ( pBtnEnableSUPP && ZGetConfiguration()->GetEtc()->EnableVoice == true)
		{
			#ifdef ENABLEDTS3
						char* mode;
			ts3client_getDefaultCaptureMode(&mode);
			...
			ts3client_getCaptureDeviceList(mode,&device);
			if (device[ZGetConfiguration()->GetEtc()->VoiceDefaultDriver] != NULL)
			ts3client_setPreProcessorConfigValue(ZGetGameClient()->scHandlerID, "denoise", (ZGetConfiguration()->GetEtc()->EnableECHO ? "true" : "false"));
			ZGetConfiguration()->GetEtc()->EnableSUPP = pBtnEnableSUPP->GetCheck();
#endif
		}
		...
		if ( pBtnEnablePPT )
		{
			//enable push to talk
			#ifdef ENABLEDTS3
			ZGetConfiguration()->GetEtc()->EnablePPT = pBtnEnablePPT->GetCheck();
			ZGetGameClient()->TsPTT(!ZGetConfiguration()->GetEtc()->EnablePPT);
#endif
		}
		MButton* pBtnEnableDVI= (MButton*)pResource->FindWidget("DisplayVoiceIcon");
		if ( pBtnEnableDVI )
		{
			ZGetConfiguration()->GetEtc()->EnableDVI = pBtnEnableDVI->GetCheck();
		}
		MButton* pDmg = (MButton*)pResource->FindWidget("DAMAGEOption");
		if(pDmg)
		{
			Z_ETC_DAMAGE_OPTION = pDmg->GetCheck();
		}
		MButton* pHPAP = (MButton*)pResource->FindWidget("HPAPOption");
		if(pHPAP)
		{
			Z_ETC_HPAP_OPTION = pHPAP->GetCheck();
		}
		MButton* pcHPAP = (MButton*)pResource->FindWidget("CLANHPAPOption");
		if(pcHPAP)
		{
			Z_ETC_CHPAP_OPTION = pcHPAP->GetCheck();
		}
		MButton* pNozzleFlare = (MButton*)pResource->FindWidget("NozzleFlareOption");
		if(pNozzleFlare)
		{
			Z_ETC_NOZZLEFLARE_OPTION = pNozzleFlare->GetCheck();
		}
		MButton* pBulletProjectiles = (MButton*)pResource->FindWidget("BulletProjectilesOption");
		if(pBulletProjectiles)
		{
			Z_ETC_BULLETPROJECTILES_OPTION = pBulletProjectiles->GetCheck();
		}
		MButton* pShotgunSparks = (MButton*)pResource->FindWidget("ShotgunSparksOption");
		if(pShotgunSparks)
		{
			Z_ETC_SHOTGUNSPARKS_OPTION = pShotgunSparks->GetCheck();
		}
		MComboBox* pComboBox = (MComboBox*)pResource->FindWidget("CrossHairComboBox");
		if (pComboBox)
		{
		...
		if(pComboBox)	
		{
			RSetFrameLimitPerSeceond(pComboBox->GetSelIndex());
...
BEGIN_IMPLEMENT_LISTENER( ZSetDefaultCharZoomListener, MBTN_CLK_MSG )
	ZIDLResource* pResource = ZApplication::GetGameInterface()->GetIDLResource();
	ZGetConfiguration()->GetVideo()->Zoom = 290;
	ZGetCamera()->m_fDist = ZGetConfiguration()->GetVideo()->Zoom;
	MSlider * pCharZoom = (MSlider*)pResource->FindWidget("CharZoom");
	if(pCharZoom)
	{
