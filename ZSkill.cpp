...
bool ZSkill::Update(float fElapsed)
{
	...
	if(ZGetGame()->GetTime()>m_fNextDamageTime) {
		...
		for(ZObjectManager::iterator i = ZGetObjectManager()->begin();i!=ZGetObjectManager()->end();i++) {
			...
			if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC && pTarget->GetTeamID() == team) continue;
...
void ZSkill::PreExecute(MUID& uidTarget, rvector& targetPos, MMatchTeam t )
{
	team = t;
...
void ZSkill::LastExecute(MUID& uidTarget, rvector& targetPos )
{
	...
	for(ZObjectManager::iterator i = ZGetObjectManager()->begin();i!=ZGetObjectManager()->end();i++) {
		...
		if(CheckRange(pTarget->GetPosition(),pTarget) && m_pDesc->CheckResist(pTarget,&fDamage)) {
			if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC && team != pTarget->GetTeamID() || ZGetGame()->GetMatch()->GetMatchType() != MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC && ZGetGame()->CanAttack(m_pOwner,pTarget)) {
				if(fDamage)
					pTarget->OnDamaged(m_pOwner,m_TargetPos,ZD_MELEE,MWT_KATANA,fDamage,m_pDesc->nModCriticalRate*.01f);
			}
			else {
...
void ZSkill::Execute(MUID& uidTarget, rvector& targetPos, MMatchTeam t)
{
	m_bEnable = true;
	...
	// 스킬 사용
	Use(uidTarget, targetPos, t);
}

void ZSkill::Use(MUID& uidTarget, rvector& targetPos, MMatchTeam t)
{
	m_nUseNum++;
	...
	if ( m_pDesc->bHitCheck)
	{
		if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC && m_pOwner->GetTeamID() != team || ZGetGame()->GetMatch()->GetMatchType() != MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC)
		{
			...
			if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC)
				ZGetGame()->m_WeaponManager.AddMagic( this, vMissilePos, vMissileDir, m_pOwner, team);
			else
	...
	else
	{
		...
		for ( ZObjectManager::iterator i = ZGetObjectManager()->begin();  i!=ZGetObjectManager()->end();  i++)
		{
			...
			// 죽은npc 는 제외
			if(pObject->IsDie() || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC && team == pObject->GetTeamID())
				continue;
			//if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC && pObject->GetTeamID() == t) continue;
			...
			if ( CheckRange( pTargetObject->GetPosition(), pObject) && m_pDesc->CheckResist( pObject, &fDamage))
			{
				...
				// 데미지를준다
				if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC && team != pObject->GetTeamID() || ZGetGame()->GetMatch()->GetMatchType() != MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC && ZGetGame()->CanAttack(m_pOwner,pObject))			
				{
