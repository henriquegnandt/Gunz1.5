...
//	m_pRocketSmokeEffect		= m_pEffectMeshMgr->Get("rocket_smoke_effect");
	...
	m_pVoiceChatIcon = m_pEffectMeshMgr->Get("ef_effectmic.elu");
	m_pBerserkerEffect[0]	= m_pEffectMeshMgr->Get("ef_berserker");
	m_pBerserkerEffect[1]	= m_pEffectMeshMgr->Get("ef_berserke1");
	m_pBerserkerEffect[2]	= m_pEffectMeshMgr->Get("ef_berserke2");
	m_pBerserkerEffect[3]	= m_pEffectMeshMgr->Get("ef_berserke3");
	m_pFlags[0] = m_pEffectMeshMgr->Get("ef_wctf_r");
	m_pFlags[1] = m_pEffectMeshMgr->Get("ef_wctf_b");
...
void ZEffectManager::Update(float fElapsed)
{
	...
	if(ZGetConfiguration()->GetVideo()->bBulletEffect == true)
...
void ZEffectManager::AddDashEffect(rvector& Target, rvector& TargetNormal, ZObject* pObj, int nDashColor) //Lucas: Dash Color
{
	...
	else if (nDashColor == 7)
	{
		sprintf(szDash, "dash_effect%d", RandomNumber(1, 7));
	}
	...
	/*if (ZGetGame()->IsInfected())
	{
	strcpy(szDash, "dash_effect1");
	}*/
	ZEffect* pNew = NULL;
...
#define SMOKE_VELOCITY				110.f
		...
		if(bRender) {
			...
			if(pCObj)
			{
				if(wtype!=MWT_SHOTGUN && wtype!=MWT_DOUBLE_SHOTGUN && pCObj->IsHero() && bSlugOutput == true)	// 샷건의 한알 한알은 탄피 생략
				{
	...
	if(bDrawTargetEffects) {
		...
		if(nTargetType==ZTT_OBJECT){	// 타겟이 맵일때
			...
			if(wtype != MWT_SHOTGUN && wtype!=MWT_DOUBLE_SHOTGUN) // 샷건의 한알 한알은 연기 생략
			{
				// 타겟 총탄 연기
			...
			// 피 분출
		/*#define TARGET_BLOOD_MAX_SCALE		100.0f
		#define TARGET_BLOOD_MIN_SCALE		20.0f
		#define TARGET_BLOOD_LIFE_TIME		500
		#define TARGET_BLOOD_VELOCITY		14.0f	// meter/sec
			...
			for(int i=0; i<3; i++){
				...
				float fSize = 1.0f+float(rand()%100)/100.0f;
				AddSmokeEffect(m_pEBSBloods[rand()%BLOOD_COUNT], p, TargetNormal*TARGET_BLOOD_VELOCITY, rvector(0,100.f,0), TARGET_BLOOD_MIN_SCALE*fSize, TARGET_BLOOD_MAX_SCALE*fSize, TARGET_BLOOD_LIFE_TIME);
				Add(pNew);
			}*/
...
void ZEffectManager::AddFlagOnBack(ZObject* pObj, int Type)
{
	ZEffect* pNew;
    pNew = new ZEffectBerserkerIconLoop(m_pFlags[Type],pObj);
...
void ZEffectManager::AddChatIcon(ZObject* pObj, bool voice)
{
	...
	public:
		ZEffectChatIconLoop(RMesh* pMesh, ZObject* pObj) 
			...
			virtual bool Draw(unsigned long int nTime)
			{
				...
				if( pChar ) {
					if(!pChar->m_dwStatusBitPackingValue.Ref().m_bVoiceChatEffect && !pChar->m_dwStatusBitPackingValue.Ref().m_bChatEffect) return false;
					if(!pChar->m_bRendered)
						return true;
					if(!pChar->IsVisible())
						return true;
					if( pObj->m_pVMesh ) {
						m_Pos = pObj->GetVisualMesh()->GetHeadPosition(!bMemEdited)+rvector(0,0,60);
	...
	ZEffect* pNew = NULL;
	pNew = new ZEffectChatIconLoop(voice ? m_pVoiceChatIcon : m_pChatIcon,pObj);
