...
struct ZLocalKillInfo
{
	...
	void AddKills() { Kills++; TotalKills++; }
	void Reset() { Kills = 0; }
};
typedef map<char*, ZLocalKillInfo*> LocalKillStreak;
typedef map<char*, int> LocalAttKillStreak;
...
//	ZWorld				*m_pWorld;
	...
	LocalKillStreak LocalKillData;
	LocalAttKillStreak LocalAttKillData, LocalVictKillData;
	vector<MUID>	ZPeerAntiHackBlocked;
	ZPeerAntiHackDef ZPeerAntiHack;
public:		
RetryArray RetryShots;
	RecvArray RecievedShots;
		MUID AttackedMeLast;
	...
	/* [[SHUFFLE_LINE]] ZGame */	int					m_render_poly_cnt;
									unsigned long int st_nLastTime[7];
...
public:
	...
	void OnExplosionMagicNonSplash(ZWeaponMagic *pWeapon, MUID uidOwner, MUID uidTarget, rvector pos, float fKnockBack, MMatchTeam nTeamID);
...
//	void OnPeerShot_Item(ZCharacter* pOwnerCharacter,float fShotTime, rvector& pos, rvector& dir,int type);
	...
	void OnPeerDead(const MUID& uidAttacker, int AddedXp, int Percent, const MUID& uidVictim, int LostXp, int LostPercent, int nAttackerStreak, int nVictimStreak);
	void OnReceiveTeamBonus(const MUID& uidChar, const unsigned long int nExpArg);
	void OnPeerDie(MUID& uidVictim, MUID& uidAttacker);
	void OnPeerDieMessage(ZCharacter* pVictim, ZCharacter* pAttacker, int nAttackerStreak, int nVictimStreak);
...
public:
	...
	void OnPeerShot_Range_Damaged(ZObject* pOwner, float fShotTime, const rvector& pos, const rvector& to, ZPICKINFO pickinfo, DWORD dwPickPassFlag, rvector& v1, rvector& v2, ZItem *pItem, rvector& BulletMarkNormal, bool& bBulletMark, ZTargetType& nTargetType, const MMatchCharItemParts sel_type);
	void OnPeerShot_Shotgun(const MMatchCharItemParts sel_type, ZCharacter* pOwnerCharacter, float fShotTime, const rvector& pos, const rvector& to);
	MTD_SHOTINFO* OnPeerShotgun_Damaged(ZObject* pOwner, float fShotTime, const rvector& pos, rvector &dir, ZPICKINFO pickinfo, DWORD dwPickPassFlag, rvector& v1, rvector& v2, ZItem *pItem, rvector& BulletMarkNormal, bool& bBulletMark, ZTargetType& nTargetType, bool& bHitEnemy, const MMatchCharItemParts sel_type, int ID, rvector to);
