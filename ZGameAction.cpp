...
bool ZGameAction::OnPeerSkill(MCommand* pCommand)
{
	...
	switch(nSkill)	{
		...
		case ZC_SKILL_UPPERCUT		: 
			OnPeerSkill_Uppercut(pOwnerCharacter);break;
			// 강베기 스플래시
		case ZC_SKILL_SPLASHSHOT	:
			OnPeerSkill_LastShot(fTime,pOwnerCharacter);break;
			// 단검 특수공격
...
//		waveCenter += _vdir * 180.f;
			...
			itor != ZGetObjectManager()->end(); ++itor)
		{
			...
			bReturnValue = !pTar->GetHistory(20, &TargetPosition, &TargetDir, fShotTime);
			if( !pTar->GetHistory(20, &TargetPosition,&TargetDir,fShotTime)) 
				PROTECT_DEBUG_REGISTER(bReturnValue)
					continue;
...
void ZGameAction::OnPeerSkill_Uppercut(ZCharacter *pOwnerCharacter)
{
		...
		itor != ZGetObjectManager()->end(); ++itor)
	{
		...
		// 적절한 위치를 얻어낼수 없으면 다음으로~
		if( !pTar->GetHistory(20, &TargetPosition,&TargetDir,fShotTime)) continue;
				...
				if( bCheck) {//팀이아닌경우만
					...
					if (fDot>0)
					{
						...
						*/
						...
						pTar->OnBlast(OwnerDir, pOwnerCharacter->GetUID());
	...
	if(pDesc)
	{
		switch(pOwnerCharacter->GetEnchantType())
		{
			...
			case ZC_ENCHANT_COLD :
				ApplyColdEnchantDamage(pTarget, pOwnerCharacter, pDesc->m_nLimitSpeed.Ref(), pDesc->m_nDelay.Ref());
				break;
...
bool ZGameAction::ApplyColdEnchantDamage(ZObject* pTarget, ZObject* pOwner, int nLimitSpeed, int nDuration)
{
	if (!pTarget || !pOwner) return false;
