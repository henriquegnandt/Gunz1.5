...
void ZRoomListBox::OnDraw( MDrawContext* pDC )
{
	...
	for( int i = 0; i < NUM_DISPLAY_ROOM; ++i )
	{
		...
		pDC->Text(MRECT( r.x, r.y+13, r.w, r.h),  m_pMapInfo[i].map_name, MAM_LEFT );
		...
		else
		{
			...
			if(m_pMapInfo[i].bIsAntiLead == false) 
			/*{
			...
			if(m_pMapInfo[i].nGravity != 100)
			{
				pDC->SetFont(MFontManager::Get("FONTa6"));
 				r.x = width + m_RoomWidth*0.65 + pressed_reposition;
				r.y = height + m_RoomHeight*0.3f + pressed_reposition;
				pDC->SetBitmapColor(255,255,255,255);
				char gravtext[50];
				sprintf(gravtext, "Gravity: %d%%", m_pMapInfo[i].nGravity);
				pDC->Text(r.x, r.y, gravtext);
			}
		}
...
bool ZRoomListBox::SetupRoomHoverInfo(int RoomID)
{
	MTextArea* pTextArea = (MTextArea*)ZGetGameInterface()->GetIDLResource()->FindWidget("Shop_ItemDescription");
	if (pTextArea)
	{
			MPOINT ptInList = MScreenToClient(this, MEvent::LatestPos);
			MRECT rcListBox = GetRect();
			MRECT rcItem = MRECT(20, 20, 200, 400);
			
				pTextArea->SetTextColor( MCOLOR(255,255,255));
				char Title[40];
				sprintf(Title, "Room: %d", m_pMapInfo[RoomID].RoomNumber);
				pTextArea->SetText(Title);

				pTextArea->AdjustHeightByContent();
				MRECT rcTextArea = pTextArea->GetRect();
				MPOINT posDesc(rcItem.x, rcItem.y);
				posDesc = MClientToScreen(this, posDesc);
				posDesc.x -= pTextArea->GetClientWidth();			// 일단 아이콘의 왼쪽으로
				...
				if (posDesc.y < 0)											// 그렇다고 화면 위로 뚫고 가면 안된다
					posDesc.y = 0;
				pTextArea->SetPosition(posDesc);
				pTextArea->SetZOrder(MZ_TOP);
				pTextArea->Show(true);
	}
	return true;
}
...
void ZRoomListBox::SetRoom(const _RoomInfoArg* pRoomInfo)
{
	...
	m_pMapInfo[nIndex].bIsAntiLead = pRoomInfo->bIsAntiLead;
	m_pMapInfo[nIndex].nGravity = pRoomInfo->nGravity;
