	...
	case MC_MATCH_GAME_DEAD:
		{
			...
			bool bSuicide = false;
			if (GameDeath.Attacker == GameDeath.Victim) bSuicide = true;

			if ((GameDeath.Attacker != MUID(0,0)) && (GameDeath.Attacker == m_uidBerserker))
			{
				if (!bSuicide)
				{
...
void ZRuleReverseBerserker::AssignBerserker(MUID& uidBerserker)
{
	...
	if (pBerserkerChar)
	{
		ZGetEffectManager()->AddBerserkerIcon(pBerserkerChar, 1);
		...
		if (!pBerserkerChar->IsDie())
		{
			...
			if ( uidBerserker == ZGetMyUID())
				ZGetGameInterface()->PlayVoiceSound( VOICE_GOT_REVERSE_BERSERKER, 2000);
			else
				ZGetGameInterface()->PlayVoiceSound( VOICE_REVERSE_BERSERKER_DOWN, 1900);
		}
