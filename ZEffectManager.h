...
#define BULLETONWALL_COUNT	2
	...
	RMesh*	m_pChatIcon;
	RMesh*	m_pVoiceChatIcon;

	RMesh*	m_pBerserkerEffect[3];
	RMesh*	m_pFlags[2];
...
//	void AddEnchantPoison(ZObject* pObj);
	...
	void AddBerserkerIcon(ZObject* pObj,int nType);
	void AddFlagOnBack(ZObject* pObj, int Type);
	void AddChatIcon(ZObject* pObj, bool voice=false);
