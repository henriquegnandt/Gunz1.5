...
void ZActor::InitFromNPCType(MQUEST_NPC nNPCType, float fTC, int nQL)
{
	if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC)
		m_pNPCInfo = ZGetNpcHelper()->GetNPCInfo(nNPCType);
	else
...
void ZActor::OnBlast(rvector &dir, MUID ATTACKER)
{
...
ZActor* ZActor::CreateActor(MQUEST_NPC nNPCType, float fTC, int nQL, MMatchTeam t, bool bForceCollRadius35)
{
...
ZOBJECTHITTEST ZActor::HitTest( const rvector& origin, const rvector& to, float fTime, rvector *pOutPos )
{
	...
	if (!ZObject::GetHistory(20, &footpos, &actor_dir, fTime)) return ZOH_NONE;
	...
	else
	{
		...
		if (m_pVMesh)
		{
			...
			headpos.z += m_Collision.GetHeight() - g_fValue5;
		}

		rvector ap,cp;
		float fDist=GetDistanceBetweenLineSegment(origin,557,to,footpos,headpos,&ap,&cp);
		float fDistToThis=Magnitude(origin-cp);
		if(fDist < (m_Collision.GetRadius()- g_fValue5))
		{
			rvector dir = to - origin;
			Normalize(dir);
...
{
	if(pAttacker && ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC && pAttacker->GetTeamID() == Team) return;
	// 맞는 사운�
