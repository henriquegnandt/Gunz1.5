...
void ZMatch::ProcessRespawn()
{
	...
	if (pCache && pCache->CheckFlag(MTD_PlayerFlags_AdminHide)) return;
	...
	case MMATCH_ROUNDSTATE_FINISH:
		{
			...
			if (IsTeamPlay())
			{
				//if(ZGetGame()->GetMatch()->GetMatchType() != MMATCH_GAMETYPE_DEATHMATCH_TEAM_TRIPLE)	{	// 3 Team DM.
				if (nArg == MMATCH_ROUNDRESULT_RED_ALL_OUT )
				{
					if( m_nTeamScore[MMT_BLUE] < m_nTeamScore[MMT_RED] && m_nTeamScore[MMT_YELLOW] < m_nTeamScore[MMT_RED])
						m_nTeamScore[MMT_YELLOW] = m_nTeamScore[MMT_BLUE] = m_nTeamScore[MMT_RED] = 0;
					else
						m_nTeamScore[MMT_BLUE]++;
				}
				else if( nArg ==  MMATCH_ROUNDRESULT_BLUE_ALL_OUT )
				{
					if( m_nTeamScore[MMT_RED] < m_nTeamScore[MMT_BLUE] && m_nTeamScore[MMT_YELLOW] < m_nTeamScore[MMT_BLUE] )
                        m_nTeamScore[MMT_YELLOW] = m_nTeamScore[MMT_BLUE] = m_nTeamScore[MMT_RED] = 0;
					else
						m_nTeamScore[MMT_RED]++;
				}				
				else if( nArg ==  MMATCH_ROUNDRESULT_YELLOW_ALL_OUT )
				{
					if( m_nTeamScore[MMT_RED] < m_nTeamScore[MMT_YELLOW] && m_nTeamScore[MMT_BLUE] < m_nTeamScore[MMT_YELLOW] )
                        m_nTeamScore[MMT_YELLOW] = m_nTeamScore[MMT_BLUE] = m_nTeamScore[MMT_RED] = 0;
					else
						m_nTeamScore[MMT_YELLOW]++;
				}
				...
				else if (nArg == MMATCH_ROUNDRESULT_DRAW)
				{
					...
					case MMATCH_ROUNDRESULT_BLUEWON:
						m_nTeamScore[MMT_BLUE]++;
						break;
					case MMATCH_ROUNDRESULT_YELLOWWON:
						m_nTeamScore[MMT_YELLOW]++;
						break;
					}
				}
