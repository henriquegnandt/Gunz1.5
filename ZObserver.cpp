...
void ZObserver::Show(bool bVisible)
{
	...
	if (bVisible)
	{
		if(ZGetGame()->GetMatch()->IsTeamPlay() && ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM_TRIPLE) 
		{
			...
			if(ZGetGame()->m_pMyCharacter->GetTeamID()==MMT_YELLOW) 
			{
				m_nType = ZOM_YELLOW;
			}
		}
...
void ZObserver::ShowDeath(MUID muid)
{
	if(ZGetGame()->IsReplay()) return;
	if (m_bVisible == true) return;
	isdeathcam = true;
	m_pCamera->SetLookMode(ZCAMERA_DEFAULT);
	if(ZGetGame()->GetMatch()->IsTeamPlay()) {
			if(ZGetGame()->m_pMyCharacter->GetTeamID()==MMT_BLUE) 
	...
	if(pCharacter)
	{
		...
		{
			SetFirstTarget();
		} else SetTarget(pCharacter);
		m_fDelay=ZOBSERVER_DEFAULT_DELAY_TIME;
		ShowInfo(true);
		m_bVisible = true;
		LastDeath = timeGetTime();
		return;
	} 
	else
	{
...
{
	if(isdeathcam == true && (timeGetTime() - LastDeath) <= 3000) return;
...
bool ZObserver::IsVisibleSetTarget(ZCharacter* pCharacter)
{
	...
	*/
	...
	if(m_nType==ZOM_YELLOW && pCharacter->GetTeamID()==MMT_YELLOW)
		return true;
...
//		TextRelative(pDC, 0.9f, 50.0f/800.0f, szModes[pCamera->GetLookMode()], true);
	...
	if ( ZGetMyInfo()->IsAdminGrade() && (ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_DEATHMATCH_TEAM2) && ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC )
	{
		// 인원수 구하기
		int nNumOfTotal=0, nNumOfRedTeam=0, nNumOfBlueTeam=0, nNumOfYellowTeam=0;
		...
		for (itor = ZGetCharacterManager()->begin(); itor != ZGetCharacterManager()->end(); ++itor)
		{
			...
			if ( (pCharacter->GetTeamID()==4) && ( !pCharacter->IsDie()))
				...
				nNumOfBlueTeam++;			
			else if ( (pCharacter->GetTeamID()==MMT_YELLOW) && ( !pCharacter->IsDie()))
				nNumOfYellowTeam++;
		}
		...
		if (ZGetGame()->GetMatch()->IsTeamPlay())
		{
			...
			/*
			backgroundcolor = MCOLOR(247,244,42, 255);
			pDC->SetColor(backgroundcolor);
			sprintf( szText, "%s:%d", "Yellow Team", nNumOfYellowTeam); 
			TextRelative( pDC, 0.92f, 65.0f/600.0f, szText, true);*/
		}
		else
		{
