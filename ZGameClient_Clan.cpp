...
void ShowClanJoinerAgreeWaitFrame(bool bVisible)
{
	if(ZGetGameInterface()->GetWaitingGames() && ZGetGameInterface()->GetLatestExclusive()!=NULL)
		return;
	if(ZGetGameInterface()->GetState() == GUNZ_LOBBY || ZGetGameInterface()->GetState() == GUNZ_STAGE || ZGetGameInterface()->GetState() == GUNZ_GAME)
	{
		ZIDLResource* pResource = ZApplication::GetGameInterface()->GetIDLResource();
		MWidget* pWidget = pResource->FindWidget("ClanJoinerAgreementWait");
		if(pWidget!=NULL)
...
void ZGameClient::OnClanAskJoinAgreement(const char* szClanName, MUID& uidClanAdmin, const char* szClanAdmin)
{
	...
	if(ZGetGameInterface()->GetState() == GUNZ_LOBBY || ZGetGameInterface()->GetState() == GUNZ_STAGE)
	{
		ZIDLResource* pResource = ZApplication::GetGameInterface()->GetIDLResource();
		...
		if(pWidget!=NULL)
		{
			static ZCOUNTDOWN countDown = {30,"ClanJoinerAgreementConfirm_Remain", "ClanJoinerAgreementConfirm",OnClanAskJoinAgreement_OnExpire};
			countDown.nSeconds=30;
			ZApplication::GetTimer()->SetTimerEvent(0, &OnTimer_CountDown, &countDown, true);
			pWidget->Show(true, true);
		}
	} 
	else if(ZGetGameInterface()->GetState() == GUNZ_GAME) 
	{
		char msg[200];
		sprintf(msg, "%s is inviting you to join %s", szClanAdmin, szClanName);
		ZChatOutput(msg, ZChat::CMT_SYSTEM);
		ZChatOutput("Type /clan accept to accept", ZChat::CMT_SYSTEM);
		static ZCOUNTDOWN countDown = {30,"ClanJoinerAgreementConfirm_Remain", "ClanJoinerAgreementConfirm",OnClanAskJoinAgreement_OnExpire};
		countDown.nSeconds=30;
		ZApplication::GetTimer()->SetTimerEvent(0, &OnTimer_CountDown, &countDown, true);
	}
...
void ZGameClient::OnClanResponseAgreedJoinClan(const int nResult)
{
	...
	if (nResult == MOK)
	{
		...
		if(ZGetGameInterface()->GetState() == GUNZ_GAME)
			ZChatOutput(ZMsg(MSG_CLAN_JOINED_NEW_MEMBER), ZChat::CMT_SYSTEM);
		else
	...
	{		
		if(ZGetGameInterface()->GetState() == GUNZ_GAME)
			ZChatOutput(ZErrStr(nResult), ZChat::CMT_SYSTEM);
		else
		ZApplication::GetGameInterface()->ShowErrorMessage( nResult );
	}
...
{
	if(find(Ignores.begin(), Ignores.end(), szSenderName) != Ignores.end()) 
		return;
	char szText[512];
...
//	mlog("OnClanResponseClanInfo : ");
	...
	pLabel = (MLabel*)pRes->FindWidget("Lobby_ClanInfoWinLose");
	pLabel->SetText(szOutput);

	sprintf(szOutput,"%d", ZGetNetRepository()->GetClanInfo()->nPoint);
	pLabel = (MLabel*)pRes->FindWidget("Lobby_ClanInfoPoints");
	pLabel->SetText(szOutput);

	sprintf(szOutput,"%d", ZGetNetRepository()->GetClanInfo()->nTotalPoint);
	pLabel = (MLabel*)pRes->FindWidget("Lobby_ClanInfoTotalPoints");
	pLabel->SetText(szOutput);

	sprintf(szOutput,"%d", pClanInfo->nRanking);
	pLabel = (MLabel*)pRes->FindWidget("Lobby_ClanInfoRanking");
	pLabel->SetText(szOutput);
	/*

	//Assassin
	sprintf(szOutput,"%d/%d",ZGetNetRepository()->GetClanInfo()->nAssassinWins,ZGetNetRepository()->GetClanInfo()->nAssassinLosses);
	pLabel = (MLabel*)pRes->FindWidget("Lobby_ClanInfoWinLoseAssassin");
	pLabel->SetText(szOutput);

	sprintf(szOutput,"%d", ZGetNetRepository()->GetClanInfo()->nAssassinPoint);
	pLabel = (MLabel*)pRes->FindWidget("Lobby_ClanInfoPointsAssassin");
	pLabel->SetText(szOutput);

	sprintf(szOutput,"%d", ZGetNetRepository()->GetClanInfo()->nAssassinTotalPoint);
	pLabel = (MLabel*)pRes->FindWidget("Lobby_ClanInfoTotalPointsAssassin");
	pLabel->SetText(szOutput);

	sprintf(szOutput,"%d", pClanInfo->nAssassinRanking);
	pLabel = (MLabel*)pRes->FindWidget("Lobby_ClanInfoRankingAssassin");
	pLabel->SetText(szOutput);


	//Gladiator
	sprintf(szOutput,"%d/%d",ZGetNetRepository()->GetClanInfo()->nGladiatorWins,ZGetNetRepository()->GetClanInfo()->nGladiatorLosses);
	pLabel = (MLabel*)pRes->FindWidget("Lobby_ClanInfoWinLoseGladiator");
	pLabel->SetText(szOutput);

	sprintf(szOutput,"%d", ZGetNetRepository()->GetClanInfo()->nGladiatorPoint);
	pLabel = (MLabel*)pRes->FindWidget("Lobby_ClanInfoPointsGladiator");
	pLabel->SetText(szOutput);

	sprintf(szOutput,"%d", ZGetNetRepository()->GetClanInfo()->nGladiatorTotalPoint);
	pLabel = (MLabel*)pRes->FindWidget("Lobby_ClanInfoTotalPointsGladiator");
	pLabel->SetText(szOutput);

	sprintf(szOutput,"%d", pClanInfo->nGladiatorRanking);
	pLabel = (MLabel*)pRes->FindWidget("Lobby_ClanInfoRankingGladiator");
	pLabel->SetText(szOutput);*/
