...
protected:
	...
	INBOX_TYPE			m_CurrentInboxType; //Steven: Inbox
...
//		MATCHCLIENT_DEFAULT_UDP_PORT, MTD_CharInfo* pCharInfo = NULL, int nTeam = 0);
	...
	void OnChannelChat(const MUID& uidChannel, char* szName, char* szChat,int nGrade, int nCid);
...
//	void OnStageLeaveBattle(const MUID& uidChar, const MUID& uidStage);
	...
	void OnStageChat(const MUID& uidChar, const MUID& uidStage, char* szChat, int nCid);
	...
	void OnUserWhisper(char* pszSenderName, char* pszTargetName, char* pszMessage, int nCid);
	...
	void AntiOnExpire(void* pBlob);	///< 기간제 아이템 사용 만료 통지받음
	void OnBirdTest();
protected:
	// 클랜관련
...
protected:
	...
	void OnStaffHelp(const char* szMsg, const char* szUsr, const int UgradeID); // Steven: Staff Command
	void OnAdminWall(const char* szMsg, const char* szCharName); // Steven: Admin Wall
public:
	ZGameClient();
	virtual ~ZGameClient();
...
public:
	...
	void StartStageList();
	void StopStageList();
	void StartInboxList(INBOX_TYPE nInboxType);
	void StopInboxList();
	...
	INBOX_TYPE		GetInboxType()			{ return m_CurrentInboxType; } //Steven: Inbox
...
#define ZPOSTVOICE(_P0, _P1)	{ MCommand* pC=ZNewCmd(CLOAK_CMD_ID(MC_GUNZ_VOICE, 12271227)); pC->AP(_P0); pC->AP(_P1); ZPostCommand(pC); }
