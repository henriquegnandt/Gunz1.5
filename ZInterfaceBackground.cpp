...
//	m_bShowMaietLogo = false;
	Helper = new ZInterfaceBackgroundHelper();
}
...
void ZInterfaceBackground::LoadMesh( void)
{
	...
	if ( m_pLogin == NULL)
	{
		...
		{
			RDummy* pDummy = *itor;
			/*if( strstr( pDummy->szName.c_str(), "camera_pos "))
			{
				Helper->Add(pDummy->Position, pDummy->Direction);
			}
			else if( stricmp( pDummy->szName.c_str(), "char_select_pos 01" ) == 0 )
			{
				m_vCamPosSt = pDummy->Position;
				m_vCamDirSt = pDummy->Direction;
			}			
			else if( stricmp( pDummy->szName.c_str(), "char_select_pos 02" ) == 0 )
			{
				m_vCamPosEd = pDummy->Position;
				m_vCamDirEd = pDummy->Direction;
			}*/
			if( stricmp( pDummy->szName.c_str(), "camera_pos 01" ) == 0 )//steven
			{
				m_vCamPosSt = pDummy->Position;
				m_vCamDirSt = pDummy->Direction;
			}
		...
		//startpos = Helper->GetStartPos();steven
	}
}
...
void ZInterfaceBackground::Draw(void)
{
	...
	switch ( m_nSceneNumber)
	{
		case  LOGIN_SCENE_FIXEDSKY :
		{
			...
			//vCamPos = Helper->GetStartPos();
			//vCamDir = Helper->GetStartDir();
...
//				ZApplication::GetSoundEngine()->PlayMusic( true);
			...
			if ( fSeed > 3.14)
			{
				m_nSceneNumber = LOGIN_SCENE_FIXEDCHAR;//steven
//				m_bShowMaietLogo = false;
			}
			...
			vCamPos = m_vCamPosEd + ( m_vCamPosSt - m_vCamPosEd) * fGain;// steven
			vCamDir = m_vCamDirEd + ( m_vCamDirSt - m_vCamDirEd) * fGain;
			/*rvector endpos = Helper->GetEndPos(RGetCameraPosition(), vCamDir);
			
			if(Helper->GetCount() != lastcount)
			{
				startpos = RGetCameraPosition();
			}*/
			//lastcount = Helper->GetCount();
			/*vCamPos = endpos + (startpos - endpos) * fGain;
			vCamDir = Helper->GetEndDir() + (Helper->GetStartDir() - Helper->GetEndDir()) * fGain;
			if(RGetCameraPosition()==endpos)
			{
				OutputDebugString("lol1\n");
			}
			if(RGetCameraPosition()>=endpos-rvector(-50.f, -50.f, -50.f))
			{
				OutputDebugString("lol2\n");
			}
			if(RGetCameraPosition()>=endpos+rvector(-50.f, -50.f, -50.f))
			{
				OutputDebugString("lol3\n");
			}
			//*/
			//if (m_bShowMaietLogo)
			//{
