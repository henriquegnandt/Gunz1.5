...
void ZPlayerMenu::SetupMenu(ZPLAYERMENU_SET nMenuSet)
{
	...
	if (nMenuSet == ZPLAYERMENU_SET_LOBBY) {
		...
		*/
		...
		if(myGrade == MCG_MASTER || myGrade == MCG_ADMIN || myGrade == MCG_SUPERADMIN) {	// 클랜 메뉴 추가
		...
		MMatchClanGrade myGrade = ZGetMyInfo()->GetClanGrade();
		if(myGrade == MCG_MASTER || myGrade == MCG_ADMIN || myGrade == MCG_SUPERADMIN)
			MPopupMenu::AddMenuItem("--------");
...
//			AddMenuItem(new ZPlayerMenuItem(ZCMD_PLAYERMENU_CLAN_GRADE_MASTER, ZMsg( MSG_MENUITEM_CLANGRADEMASTER)));		// 잠시 봉인
			AddMenuItem(new ZPlayerMenuItem(ZCMD_PLAYERMENU_CLAN_GRADE_SUPERADMIN, ZMsg( MSG_MENUITEM_CLANGRADESUPERADMIN)));
		} 
		if (myGrade == MCG_MASTER || myGrade == MCG_SUPERADMIN)
		{
...
bool ZPlayerMenuListener::OnCommand(MWidget* pWidget, const char* szMessage)
{
	...
	case ZCMD_PLAYERMENU_CLAN_GRADE_ADMIN:
		...
		return true;
	case ZCMD_PLAYERMENU_CLAN_GRADE_SUPERADMIN:
		ZPostRequestChangeClanGrade(ZGetMyUID(),pMenu->GetTargetName(),MCG_SUPERADMIN);
		return true;
