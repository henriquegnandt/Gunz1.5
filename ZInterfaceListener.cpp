		...
		if(MWidget::IsMsg(szMessage, MBTN_CLK_MSG)==true)
		{
			...
			if ( pServer)
			{
				...
				if ( pServer->nType == 7)	// Debug server Type1을 Type7로 변경
				{
					...
					ZPostConnect(pServer->szAddress, pServer->nPort);		//Steven: Debug server
				}
				else
...
// Channel
BEGIN_IMPLEMENT_LISTENER(ZGetInboxFrameCallerListener, MBTN_CLK_MSG) //Steven: Inbox
	ZIDLResource* pResource = ZGetGameInterface()->GetIDLResource();
	MWidget* pFindWidget = pResource->FindWidget("Inbox");
	if(pFindWidget!=NULL) pFindWidget->Show(true, true);

	INBOX_TYPE nCurrentInboxType = ZGetGameClient()->GetInboxType();
	ZGetGameInterface()->InitInboxFrame(nCurrentInboxType);
	ZGetGameClient()->StartInboxList(nCurrentInboxType);
END_IMPLEMENT_LISTENER()
	
BEGIN_IMPLEMENT_LISTENER(ZInboxListCaller, MLB_ITEM_DBLCLK)
	ZIDLResource* pResource = ZGetGameInterface()->GetIDLResource();
	MListBox* pInboxList = (MListBox*)pResource->FindWidget("LISTINBOX");
	ZInboxListItem* pItem = (ZInboxListItem*)pInboxList->GetSelItem();
	ZPostInboxMessage(pItem->GetUID());
END_IMPLEMENT_LISTENER()

BEGIN_IMPLEMENT_LISTENER(ZGetInboxCloseButtonListener, MBTN_CLK_MSG)
	ZIDLResource* pResource = ZGetGameInterface()->GetIDLResource();
	MWidget* pWidget = pResource->FindWidget("Inbox");
	if(pWidget!=NULL) pWidget->Show(false);
END_IMPLEMENT_LISTENER()

BEGIN_IMPLEMENT_LISTENER(ZGetCustomMusicRefresh, MBTN_CLK_MSG)
	ZApplication::GetSoundEngine()->LoadCustomMusic();
END_IMPLEMENT_LISTENER()
...
BEGIN_IMPLEMENT_LISTENER(ZGetStagePlayerListPrevListener, MBTN_CLK_MSG)
{
	...
	if (pWidget->GetMode() == ZPlayerListBox::PLAYERLISTMODE_STAGE_FRIEND || pWidget->GetMode() == ZPlayerListBox::PLAYERLISTMODE_STAGE_IGNORE) {
		int iStart = pWidget->GetStartItem();
		if (iStart > 0) 
			pWidget->SetStartItem(iStart-1);
		return true;
...
BEGIN_IMPLEMENT_LISTENER(ZGetArrangedTeamGameListener, MBTN_CLK_MSG)
	...
	if(pWidget!=NULL)
	{
		MButton* pButton = (MButton*)pResource->FindWidget("ClanAntiLead");
		if (pButton) pButton->Show(false, false);
		MLabel* pLabel = (MLabel*)pResource->FindWidget("AntiLeadLabel");
		if(pLabel) pLabel->Show(false, false);
		MComboBox* pCombo = (MComboBox*)pResource->FindWidget("CWGameType");
		if(pCombo) pCombo->Remove(1);
		pWidget->Show(true,true);
...
//#ifdef _DEBUG	// 1명테스트
				else if (nCount == 0)
				{
					...
					pButton = (MButton*)pResource->FindWidget("ClanAntiLead");// Steven: ClanWar AntiLead
					if ((pButton) && (pButton->GetCheck()))
					{
						AntiLead = 1;
					}				
					...
					ZPostLadderRequestChallenge(ppMember, 1, nBalancedMatching, AntiLead, GameType);
			}
//#endif
				else
				{
