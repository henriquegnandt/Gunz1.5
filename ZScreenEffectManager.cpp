...
void ZScreenEffectManager::Destroy()
{
	...
	SAFE_DELETE(m_pWeaponIcons[MWT_DOUBLE_GREAT_SWORD]);
	...
	SAFE_DELETE(m_pWeaponIcons[MWT_DOUBLE_SHOTGUN]);
	SAFE_DELETE(m_pWeaponIcons[MWT_SAWED_SHOTGUN]);

	SAFE_DELETE(m_pWeaponIcons[MWT_RIFLE]);
	SAFE_DELETE(m_pWeaponIcons[MWT_DOUBLE_RIFLE]);
	...
	SAFE_DELETE(m_pWeaponIcons[MWT_CLAYMORE]);
...
#define END_(x) { _end_time = timeGetTime(); float f_time = (_end_time - _begin_time) / 1000.f; mlog("%s : %f \n", x,f_time ); }
	...
	m_pWeaponIcons[MWT_DOUBLE_GREAT_SWORD]	= new ZScreenEffect(m_pEffectMeshMgr->Get("ef_in_icon_d_sword.elu"));
	...
	m_pWeaponIcons[MWT_DOUBLE_SHOTGUN]	= new ZScreenEffect(m_pEffectMeshMgr->Get("ef_in_icon_d_shotgun.elu"));
	...
	m_pWeaponIcons[MWT_DOUBLE_RIFLE]	= new ZScreenEffect(m_pEffectMeshMgr->Get("ef_in_icon_d_rifle.elu"));
	...
	m_pWeaponIcons[MWT_SNIFER]			= new ZScreenEffect(m_pEffectMeshMgr->Get("ef_in_icon_snifer.elu"));
	m_pWeaponIcons[MWT_MED_KIT]			= new ZScreenEffect(m_pEffectMeshMgr->Get("ef_in_icon_medikit.elu"));
	m_pWeaponIcons[MWT_CLAYMORE]		= new ZScreenEffect(m_pEffectMeshMgr->Get("ef_in_icon_grenade.elu"));
...
void ZScreenEffectManager::DrawCombo()
{
	...
	for(int i=0;i<COMBOEFFECTS_COUNT;i++)
	{
		if(m_pComboEffects[i] && m_pComboEffects[i]->GetVMesh()) {
			...
			// Lucas: VMesh NULL and GetFrameInfo fix
...
void ZScreenEffectManager::SetCombo(int nCombo)
{
	...
	char buffer[32];
	// Lucas: sprintf_s prevent buffer overrun
...
bool ZScreenEffectManager::CreateQuestRes()
{
	...
	/*	m_pQuestEffectMeshMgr = new RMeshMgr();
	if(m_pQuestEffectMeshMgr->LoadXmlList("interface/screeneffects_quest.xml")==-1) {
	mlog("quest combat list loding error\n");
	SAFE_DELETE(m_pQuestEffectMeshMgr);
	return false;
	}*/
	...
	m_pArrow[0] = new ZScreenEffect(m_pQuestEffectMeshMgr->Get("arrow"));
	m_pArrow[1] = new ZScreenEffect(m_pQuestEffectMeshMgr->Get("arrow1"));
...
void ZScreenEffectManager::DestroyQuestRes()
{
	...
	if (m_pArrow[0])
	{
		SAFE_DELETE(m_pArrow[0]);
	}
	if (m_pArrow[1])
	{
		SAFE_DELETE(m_pArrow[1]);
	}
	//	if (m_pKO)
	//	{
	//		SAFE_DELETE(m_pKO);
...
void ZScreenEffectManager::DrawArrow(rvector& vTargetPos, float fOffsetY, float OffsetX, int Type)
{
	...
	if (m_pArrow)
	{
		m_pArrow[Type]->DrawCustom(0, rvector(OffsetX, fOffsetY, 0.0f), fAng);
	}
}
...
{
	if (ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_DEATHMATCH_TEAM2 || ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC ) return;
