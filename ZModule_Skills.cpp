...
void ZModule_Skills::PreExcute(int nSkill,MUID uidTarget,rvector targetPosition, MMatchTeam t)
{
	if(nSkill<0 || nSkill>=MAX_SKILL) return;
	m_Skills[nSkill].PreExecute(uidTarget,targetPosition, t);
}

void ZModule_Skills::Excute(int nSkill,MUID uidTarget,rvector targetPosition, MMatchTeam t)		// 실행한다
{
	if(nSkill<0 || nSkill>=MAX_SKILL) return;
	m_Skills[nSkill].Execute(uidTarget,targetPosition, t);
