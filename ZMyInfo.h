...
public:
	...
	bool				IsAdminGrade() {
		if ( (GetUGradeID() == MMUG_EVENTMASTER) || (GetUGradeID() == MMUG_GM) || (GetUGradeID() == MMUG_ADMIN) || (GetUGradeID() == MMUG_DEV)  || (GetUGradeID() == MMUG_TRAIL)|| (GetUGradeID() == MMUG_HEADGM))
			return true;
		else
			return false;
	}
	...
	// set 씨리즈
	void SetXP(__int64 nXP)			{ m_nXP = nXP; }
