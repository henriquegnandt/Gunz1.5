...
//	int					m_nSelectedCharacter;
	...
	bool GetWaitingGames() {return m_bWaitingArrangedGame; }
protected:
...
//	void SetupPlayerListTab();
	...
	void InitPlayerWarsUI(bool bClanBattleEnable);
	void InitDuelTournamentLobbyUI(bool bEnableDuelTournamentUI);
	void InitInboxFrame(INBOX_TYPE nChannelInboxType);//Steven: Inbox
...
// 릴레이맵
	...
	__forceinline ZNpcHelper* GetNpcHelper();
	__forceinline ZNpcHelper* GetNpcHelperExactly()				{ return &m_NpcHelper; }
	...
	__forceinline ZZombie* GetZombieExactly()				{ return &m_Zombie; }
...
__forceinline ZNpcHelper* ZGameInterface::GetNpcHelper()
{
	return static_cast<ZNpcHelper*>(&m_NpcHelper);
}
__forceinline ZBaseQuest* ZGameInterface::GetQuest()
{
	if (m_pGame && m_pGame->GetMatch())
	{
		...
		else if (m_GameTypeManager.IsZombieOnly( gameType ))
			return static_cast<ZBaseQuest*>(&m_Zombie);
	}
...
inline char* GetCountryFlag(char* flag)
{
	char out[40] = "";
	sprintf(out, "flag_%s.png", flag);
	return out;
}
