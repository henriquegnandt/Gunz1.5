...
public:
	ZActor();
	virtual ~ZActor();
	static ZActor* CreateActor(MQUEST_NPC nNPCType, float fTC, int nQL, MMatchTeam t = MMT_ALL, bool bForceCollRadius35=false);
	...
	MMatchTeam GetTeam() { return Team; }
public:
	...
	virtual void OnBlast(rvector &dir, MUID ATTACKER);
...
inline const MTD_NPCINFO* GetMyActorServerNPCInfo( const MQUEST_NPC nNPCID )
{
	...
	{
		if( NULL == ZGetNpcHelper() )
		{
			return NULL;
		}
		return ZGetNpcHelper()->GetNPCInfoFromServerMgr().GetNPCInfo( nNPCID );
	}
	else
	{
		if( NULL == ZGetQuest() )
