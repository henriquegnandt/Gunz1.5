...
////////////////////////////////////////////////////////////////////////////////////////////////////
#else // _BIRDSOUND
void ZSoundEngine::PlayBackRecorded()
{
}
...
//PenguinGuy
bool ZSoundEngine::HasMusicStopped()
{
	return m_bMusicEnded;
}
...
void ZSoundEngine::LoadCustomMusic() 
{
	...
	do
	{
		if( nTempSkipToTwo >= 2 && hFile != INVALID_HANDLE_VALUE && (strstr(w32Data.cFileName, ".mp3") || strstr(w32Data.cFileName, ".wav") || strstr(w32Data.cFileName, ".wma") || strstr(w32Data.cFileName, ".ogg")))
		{
...
bool ZSoundEngine::OpenCustomMusic( const char* szFileNameDir, const char* szFileName )
{
	...
	//mlog( "[Testing OpenCustomMusic] - Previos File Name: %s --- New File Name: %s\n", m_szOpenedMusicName, szFileName );
...
bool ZSoundEngine::PlayAudio(int nPlayNumb)
{
	...
	if(ZGetConfiguration()->GetAudio()->bNotfications == true)
	{
		char szName[125];
		sprintf(szName, "(MP:%d/%d)Playing: %s.", (nPlayNumb + 1), m_nNumOfMusics, m_szCustomMusicNames[nPlayNumb]);
...
void ZSoundEngine::RecordControl(ZRecordAudioControl Audio, char*Buffer)
{
}

bool ZSoundEngine::Control(ZAudioControl Audio)
{
	int nPlayNumb = 0;
	switch (Audio) 
	{
		case Play:
			if(ZGetConfiguration()->GetAudio()->bRandom == true) 
			{
				nPlayNumb = RandomNumber(0, m_nNumOfMusics);
			}
			break;
		case Next:
			if(ZGetConfiguration()->GetAudio()->bRandom == true) 
			...
			{
				if((m_nLastSongIndex+1) >= m_nNumOfMusics) 
				{
					nPlayNumb = 0;
				} 
				else 
				{
					nPlayNumb = (m_nLastSongIndex + 1);
				}
			}
			break;
		case Prv:
			...
			{
				if((m_nLastSongIndex - 1) <= 0) 
				{
					nPlayNumb = m_nNumOfMusics-1;
				} 
				else 
				{
					nPlayNumb = m_nNumOfMusics;
				}
			}
			break;
		case Pause:
			m_bPaused = !m_bPaused;
			ZGetSoundFMod()->SetPaused(m_bPaused);
			return true;
			break;
	}
	PlayAudio(nPlayNumb);
...
void ZSoundEngine::MusicEndCallback(void* pCallbackContext)
{
	...
	{
		pSoundEngine->Control(Next);
		pSoundEngine->PlayMusic( false );
	}
}
...
void ZSoundEngine::PlaySEFire(MMatchItemDesc *pDesc, float x, float y, float z, bool bPlayer)
{
	...
	if( pDesc->m_nType.Ref() == MMIT_RANGE || pDesc->m_nType.Ref() == MMIT_CUSTOM )
	{
		...
		{
			char szBuffer[64];
			sprintf(szBuffer, "%s%s", szSndName, ZGetConfiguration()->GetEtc()->b3dsound?"_optional_2d":"_2d");
#ifdef _SOUND_LOG
			mlog("%s stereo 2d sound is played..\n",szBuffer);
#endif
			PlaySoundElseDefault(szBuffer,szSndName,rvector(x,y,z),bPlayer);
			return;
		}
...
{
 char m_stSndFileName[MAX_BGM][64] = {"CharSelect.mp3", 
            "Lobby.mp3", 
            "InGame2.mp3", 
            "InGame4.mp3", 
            "InGame3.mp3",
	...
	char BGM[100] = "";
	sprintf(BGM, "Sound/bgm/%s", m_stSndFileName[nRealBgmIndex]);
	return BGM;
}
...
bool ZSoundEngine::OpenMusic(int nBgmIndex, MZFileSystem* pfs)
{
	...
	return OpenCustomMusic(szFileName, szFileName);
