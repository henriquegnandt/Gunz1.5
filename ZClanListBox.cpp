			...
			: MWidget(szName, pParent, pListener)
{
	...
	AntiLead = MBitmapManager::Get("icon_antilead.png");
	Lead = MBitmapManager::Get("icon_lead.png");*/
...
void ZClanListBox::OnDraw( MDrawContext* pDC )
{
	...
	for(int i=0;i<NUM_DISPLAY_CLAN;i++) {
		...
		ZCLANINFO *pInfo = m_pClanInfo+i;
		if(!pInfo->bEmpty) {
			if(m_pRoomFrame && TeamDeathMatch && Assassinate && Gladiator && AntiLead && Lead)
			{
				// 프레임
				pDC->SetBitmap(m_pRoomFrame);
				int y = (int)(fRatio * (m_pRoomFrame->GetHeight() + nGap ) * b) ;
				int x = 0;
				j++;
				if(j==2)
				{
					x = (int)(fRatio * m_pRoomFrame->GetWidth()/2) + 2;
					b++;
					j = 0;
				}
				pDC->Draw(x, y , (int)(fRatio * m_pRoomFrame->GetWidth()/2), (int)(fRatio * 32));
				...
				case 0:
					...
					break;
				case 1:
					pDC->SetBitmap(Assassinate);
					break;				
				case 2:
					pDC->SetBitmap(Gladiator);
					break;
				}
				pDC->Draw(x, y, m_pRoomFrame->GetHeight(), m_pRoomFrame->GetHeight());
				pInfo->bIsAntiLead ? pDC->SetBitmap(AntiLead) : pDC->SetBitmap(Lead);
				pDC->Draw(x + m_pRoomFrame->GetHeight(), y, m_pRoomFrame->GetHeight(), m_pRoomFrame->GetHeight());
				pDC->SetColor(MCOLOR(0xffffffff));
				char szBuffer[256];
				sprintf(szBuffer,"%d Players Queueing.",pInfo->nPlayers );
				if(x>=1)
				pDC->Text((int)(fRatio*248) , (int)(y + fRatio*10) , szBuffer);
				else
				pDC->Text((int)(fRatio*65) , (int)(y + fRatio*10) , szBuffer);
			}
		}
	}
}*/
...
void ZClanListBox::SetInfo(int nIndex, int nEmblemID, const char *szName, int nPlayers/*, bool bIsAntiLead, int GameType*/)
{
