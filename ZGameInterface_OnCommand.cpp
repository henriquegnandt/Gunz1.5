...
bool ZGameInterface::OnCommand(MCommand* pCommand)
{
	...
	if ((ZGetGame() == NULL))
	{//check call stack where is this originating from what is called first
	...
	if (pCommand->GetID() == MC_QUEST_GAME_INFO)
	{
		...
		else if (ZGetGameTypeManager()->IsZombieOnly(pQuestGameInfo->eGameType))
		{
			ZGetZombieExactly()->OnCommand(pCommand);
			return true;
		}
	...
	{
	/*case MC_MATCH_INBOX_RESPONSE_INFO://Steven: Inbox
		{
			MCommandParameter* pParam = pCommand->GetParameter(0);
			...
			MListBox* pWidget = (MListBox*)pResource->FindWidget("LISTINBOX");
			pWidget->RemoveAll();
			for (int i=0; i<nCount; ++i)
			{
				InboxInfo *pInboxInfo = reinterpret_cast<InboxInfo*>(MGetBlobArrayElement(pBlob, i));
				if (!pInboxInfo) { _ASSERT(0); break; }
				pWidget->Add(new ZInboxListItem(pInboxInfo->nUID, pInboxInfo->nInboxType, pInboxInfo->SenderName, pInboxInfo->Title, pInboxInfo->sysTime)); 
			}
		}
		break;*/
/*	case MC_MATCH_INBOX_MESSAGE_RESPONSE:
		{
			char Msg[512];
			pCommand->GetParameter(Msg, 0, MPT_STR, sizeof(Msg));
			ZGetGameInterface()->ShowMessage(Msg);
		}
		break;*/
	case MC_MATCH_RESPONSE_ACCOUNT_CHARLIST:
		{
			if(ZApplication::GetGameInterface()->GetState() != GUNZ_LOBBY && ZApplication::GetGameInterface()->GetState() != GUNZ_CHARCREATION && ZApplication::GetGameInterface()->GetState() != GUNZ_CHARSELECTION && ZApplication::GetGameInterface()->GetState() != GUNZ_GREETER && ZApplication::GetGameInterface()->GetState() != GUNZ_DIRECTLOGIN && ZApplication::GetGameInterface()->GetState() != GUNZ_LOGIN) return true;
			MCommandParameter* pParam = pCommand->GetParameter(0);
			if(pParam->GetType()!=MPT_BLOB) break;
			void* pCharListBlob = pParam->GetPointer();
			int nCount = MGetBlobArrayCount(pCharListBlob);
			int ServerPeerPort = 0;
			
			pCommand->GetParameter(&ServerPeerPort, 1, MPT_INT);
			// 캐릭터 정보 가져오기
			ZCharacterSelectView::OnReceivedAccountCharInfo(pCharListBlob);
			m_spGameClient->SetServerPeerPort(ServerPeerPort);
			// 캐릭터가 한명도 없으면 메시지 출력
	...
	case MC_MATCH_RESPONSE_SELECT_CHAR:
		{
			...
			if (nResult == MOK)
			{
				...
				*/
				...
				if (((ZGetGameClient()->GetServerMode() != MSM_CLANTEST)  && (!ZGetMyInfo()->IsClanJoined())) || ((ZGetGameClient()->GetServerMode() != MSM_CLAN) && (!ZGetMyInfo()->IsClanJoined())))	{
