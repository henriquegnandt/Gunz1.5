...
//	pDC->FillRectangle(rr.x,rr.y+1,rr.w,rr.h);
	...
	if( pm == ZPlayerListBox::PLAYERLISTMODE_STAGE || pm == ZPlayerListBox::PLAYERLISTMODE_STAGE_FRIEND || pm == ZPlayerListBox::PLAYERLISTMODE_STAGE_IGNORE)
		nMode = 1;
...
void ZPlayerListBox::InitUI(PLAYERLISTMODE nMode)
{
	...
	// 필드 개수테이블 
	const int nFields[PLAYERLISTMODE_END] = { 6,6,2,2,2,2,4,4 }; 
	...
	switch(m_nMode) {
		...
		case PLAYERLISTMODE_STAGE_IGNORE: 	SetupButton("pltab_ignores_on.tga","pltab_ignores_off.tga");break;
...
void ZPlayerListBox::SetMode(PLAYERLISTMODE nMode)
{
	...
	if(nMode==PLAYERLISTMODE_CHANNEL) {
		...
		if (ZGetGameClient()->IsConnected())
			ZPostFriendList();
	} else if(nMode==PLAYERLISTMODE_CHANNEL_IGNORE || nMode==PLAYERLISTMODE_STAGE_IGNORE)
	{
			if (ZGetGameClient()->IsConnected())
			ZPostIgnoreList();
...
//	m_pButton->SetBounds(1,1,m_Rect.w*.8f,m_nItemHeight+1);
	...
	case PLAYERLISTMODE_CHANNEL:
		{
			m_Fields.Get(0)->nTabSize = 23*fA;	//icon + 3
			m_Fields.Get(1)->nTabSize = 24*fA;	//level 16
			m_Fields.Get(2)->nTabSize = 23*fA;	//icon (duel tournament grade)
			m_Fields.Get(3)->nTabSize = 94*fA;	//name + 3
	...
	case PLAYERLISTMODE_STAGE:
		{
			...
			m_Fields.Get(1)->nTabSize = 24*fA;	//level
			m_Fields.Get(2)->nTabSize = 23*fA;	//icon (duel tournament grade)
	...
	case PLAYERLISTMODE_CHANNEL_FRIEND:
		...
		break;
	case PLAYERLISTMODE_STAGE_IGNORE:
	case PLAYERLISTMODE_CHANNEL_IGNORE:
		{
			m_Fields.Get(0)->nTabSize = 23*fA;	//icon
			m_Fields.Get(1)->nTabSize = 72*fA;	//name
...
void ZPlayerListBox::AddPlayer(MUID& puid, ePlayerState state, int  nLevel,char* szName, char *szClanName, unsigned int nClanID, MMatchUserGradeID nGrade, int duelTournamentGrade )
{
	...
	char* szRefName = NULL;
	MCOLOR _color;
    if(IsAdminGrade(nGrade) && nGrade != MMUG_EVENTMASTER)
		sprintf(szLevel,"--");
	else
	...
	//if(ZGetGame()->GetUserGradeIDColor(nGrade,_color,szName))
...
void ZPlayerListBox::AddPlayer(MUID& puid, MMatchObjectStageState state, int nLevel, char* szName, char* szClanName, unsigned int nClanID, bool isMaster, MMatchTeam nTeam, int duelTournamentGrade)
{
	...
	if(IsAdminGrade(pObjCache->GetUGrade()) && pObjCache->GetUGrade() != MMUG_EVENTMASTER)
		sprintf(szLevel,"--");
	else
	sprintf(szLevel,"%2d",nLevel);
	szRefName = szName;
...
//	char sp_name[256];
	...
	switch(nGrade) {
		...
		case MCG_ADMIN	: 
			...
			break;
		case MCG_SUPERADMIN	: 
			sprintf(szGradeName, ZMsg( MSG_WORD_CLAN_SUPERADMIN));
			bSpUser = true;
			break;
...
bool ZPlayerListBox::OnEvent(MEvent* pEvent, MListener* pListener)
{
	...
	if(pEvent->nMessage==MWM_RBUTTONDOWN) {
		if(rtClient.InPoint(pEvent->Pos)==true) {
			...
			/*	if((gid==MMUG_GM) || (gid==MMUG_ADMIN) || (gid==MMUG_DEV) || (gid==MMUG_TRAIL) || (gid==MMUG_HEADGM)) { // 개발자나 관리자라면..
					bShow = false;
				}*/
