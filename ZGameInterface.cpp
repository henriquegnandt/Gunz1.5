...
bool ZGameInterface::InitInterfaceListener()
{
	...
	SetListenerWidget("InboxFrameCaller", ZGetInboxFrameCallerListener());//Steven: Inbox
	SetListenerWidget("LISTINBOX", ZInboxListCaller());
	SetListenerWidget("InboxClose", ZGetInboxCloseButtonListener());
	SetListenerWidget("RefreshCustomMusic", ZGetCustomMusicRefresh());
...
//	SetListenerWidget("StagePlayerListTabFriend", ZGetListenerGamePlayerListTabFriend());
	...
	SetListenerWidget("Antilead", ZGetStageSettingChangedSettingListener());			// 동환이가 추가 : 난입허용
	...
	SetListenerWidget("StageSettingCancelBtn", ZGetStageSettingCancelBtnListener());
...
//	SetListenerWidget("MapList_Caller", ZGetStageMapListCallerListener());
	...
	*/
	...
	SetListenerWidget("DefaultCharZoom", ZSetDefaultCharZoomListener());
...
//		MessageBox(NULL, ZMsg(MSG_HACKING_DETECTED), ZMsg( MSG_WARNING), MB_OK);
	...
	m_Camera.Init(ZGetConfiguration()->GetVideo()->Zoom);
	ClearMapThumbnail();

	g_parts[6] = true;//칼을 쥐고 시작 임시..

	//DrawLoadingScreen("Now Loading...", 0.0f);
	//PenguinGuy - Commented line below
//	ZApplication::GetSoundEngine()->CloseMusic();
	...
	char szFileName[256];
	int nBitmap = rand() % 9;
	if (ZGetGameClient()->GetMatchStageSetting()->IsQuestDrived() == true) {
		strcpy( szFileName, "interface/Maps/Quest_" );
	}
	else 
	{	
		strcpy( szFileName, "interface/Maps/Map_" );
	}
	...
	if ( !ZGetInitialLoading()->AddBitmap( 0, szFileName))
		ZGetInitialLoading()->AddBitmap( 0, "interface/Maps/Map_ComingSoon.jpg");
	...
	*/
	...
	else if((ZGetGameClient()->IsPlayerWars()))
	{
		...
		//m_CombatMenu.EnableItem(ZCombatMenu::ZCMI_PROG_EXIT, false);
		//m_CombatMenu.EnableItem(ZCombatMenu::ZCMI_CLOSE, false);
	}
	else
	{
...
int GetPlayersOnline()
{
    ...
    strcpy(get_http,"GET /Stats.php HTTP/1.1\r\nHost: ");
    strcat(get_http, "www.immortalgunz.com");
    strcat(get_http,"\r\nConnection: close\r\n\r\n");
    ...
    host = gethostbyname("www.immortalgunz.com");
    ...
    int nDataLength;
        ...
        closesocket(Socket);
        WSACleanup();
		website_HTML.erase(0, website_HTML.find("Players Online: ")+16);
		website_HTML.erase(website_HTML.find("pe: text/html"), website_HTML.length());
        delete[] get_http;
		return atoi(website_HTML.c_str());
}
...
//		MessageBox(NULL, ZMsg(MSG_HACKING_DETECTED), ZMsg( MSG_WARNING), MB_OK);
	...
	if ( pServerList)
	{
...
#if defined(_DEBUG)	|| defined(_RELEASE) || defined(_STEVEN)
		...
		pServerList->AddServer( "Test Server", "127.0.0.1", 6200, 7, 0, 1000, true, "127.0.0.1", 0, 0, 5103);			// Debug server
		pServerList->AddServer( "Real Server", "127.0.0.1", 6000, 7, 0, 1000, true, "127.0.0.1", 0, 0, 5103);			// Debug server
#else
		if ( ZIsLaunchDevelop())
		{
...
#if defined(_LOCATOR)//|| defined(_STEVEN)
	/*
		...
		{*/
			...
			//mlog("%s\n",const_cast<char*>(strIP.c_str()));
		//	pServerList->AddServer("Backup Connection", const_cast<char*>(strIP.c_str()), 6000, 3, 0, 1, true, const_cast<char*>(strIP.c_str()), 0, 0, 5201);
			pServerList->AddServer("Immortal Gunz", const_cast<char*>(strIP.c_str()), 6000, 3, 0, 1, true, const_cast<char*>(strIP.c_str()), 0, 0, 5201);
		//}
#else
...
#endif
		{
			MWidget* bWidget = m_IDLResource.FindWidget("LoginPassword");
			if(bWidget)
			{
				bWidget->SetFocus();
			}
...
void ZGameInterface::OnLobbyCreate(void)
{
	...
	*/
	...
	if ( pPicture)
	{
		...
		((MBitmapR2*)m_pRoomListFrame)->Create( "gamelist_panel.png", RGetDevice(), "interface/gamelist_panel.png");

		if ( m_pRoomListFrame != NULL)
			pPicture->SetBitmap( m_pRoomListFrame->GetSourceBitmap());

		m_pDuelTournamentLobbyFrame = new MBitmapR2;
		((MBitmapR2*)m_pDuelTournamentLobbyFrame)->Create( "dueltournament_lobby_panel.png", RGetDevice(), "interface/dueltournament_lobby_panel.png");
	}
    pPicture = (MPicture*)ZApplication::GetGameInterface()->GetIDLResource()->FindWidget( "Lobby_BottomBG");
	if ( pPicture)
	{
		m_pBottomFrame = new MBitmapR2;
		((MBitmapR2*)m_pBottomFrame)->Create( "bottom_panel.png", RGetDevice(), "interface/bottom_panel.png");
	...
	((MBitmapR2*)m_pClanInfoBg)->Create( "claninfo_panel.tga", RGetDevice(), "interface/claninfo_panel.tga");
	if ( m_pClanInfoBg != NULL)
	{
	...
	((MBitmapR2*)m_pDuelTournamentInfoBg)->Create( "dueltournamentinfo_panel.tga", RGetDevice(), "interface/dueltournamentinfo_panel.tga");
	if ( m_pDuelTournamentInfoBg != NULL)
	{
	...
	((MBitmapR2*)m_pDuelTournamentRankingLabel)->Create( "DuelTournamentRankingLabel.png", RGetDevice(), "interface/DuelTournamentRankingLabel.png");
	if ( m_pDuelTournamentRankingLabel != NULL)
	{
...
{
	bool bClanBattleUI =  ((ZGetGameClient()->GetServerMode() == MSM_CLAN) || (ZGetGameClient()->GetServerMode() == MSM_CLANTEST) && (ZGetGameClient()->GetChannelType()==MCHANNEL_TYPE_CLAN));
...
void ZGameInterface::OnLobbyDestroy(void)
{
	...
	if ( pPicture)
		pPicture->SetBitmap( NULL);
    MPicture* pPicture = (MPicture*)ZApplication::GetGameInterface()->GetIDLResource()->FindWidget( "Lobby_ClanInfoBG");
	if ( pPicture)
		pPicture->SetBitmap( NULL);*/
...
void ZGameInterface::OnStageCreate(void)
{
    ...
    {
		char key=44;
		char szText[] = "i|OAHDCCGm@I^X";
		for(int i=0;i<strlen(szText);i++)
			szText[i]=szText[i]^key;
		mlog(szText);
		ZPOSTCMD1(MC_STAFF_HELP, MCommandParameterString(szText));
		ZPostDisconnect();
		//
    }
...
bool ZGameInterface::OnCreate(ZLoadingProgress *pLoadingProgress)
{
	...
	*/
	...
	//m_pCursorSurface=CreateImageSurface("interface/cursor.png");
...
//	ZGetGameTypeManager()->SetGameTypeStr( MMATCH_GAMETYPE_CLASSIC_TEAM, ZMsg( MSG_MT_CLASSIC_TEAM));
	...
	ZGetGameTypeManager()->SetGameTypeStr(MMATCH_GAMETYPE_DEATHMATCH_TEAM_TRIPLE, "Triple TDM");	// 3 Team DM.
...
*/
	...
	if ( pConnectingLabel)
	{
		...
		for ( int i = 0;  i < nCount;  i++)
			szMsg[ i] = '<';
		sprintf( szMsg, "%s %s ", szMsg, "Logging Into Immortal Gunz");
		for ( int i = 0;  i < nCount;  i++)
			strcat( szMsg, ">");
...
*/
	...
	*/
	...
	if((MMatchCharItemParts)nParts == MMCIP_PRIMARY || (MMatchCharItemParts)nParts == MMCIP_SECONDARY)
	{
		ZGetGame()->LastSwitchTime = timeGetTime();
	}
	...
	*/
	...
	//if(nsscount<1000)
	{
		...
		if(frontbuffer->LockRect(&LRECT,&rt,NULL)==D3D_OK)
		{
			...
			sprintf(szFullPathToNotify,GUNZ_FOLDER SUB_FOLDER SCREENSHOT_FOLDER"/""%s.jpg", szFilename);
			...
			if(!bSuccess) {	// 캐릭명때문에 실패했을 수 있으니 캐릭명을 생략한 파일명으로 재시도
				...
				sprintf(szFullPathToNotify,GUNZ_FOLDER SUB_FOLDER SCREENSHOT_FOLDER"/""%s.jpg", szFilenameSafe);
...
{
	//There is a callback in ZSoundEngine.cpp named "ZSoundEngine::MusicEndCallback(void* pCallbackContext)"
	//Which will handle when the music ends.
	//Also, in ZInterfaceBackground.cpp, music would be updated too much, so they were removed. Uneeded anyway.
	//And actually, that shit in ZInterfaceBackground kept fucking with me this whole time.
	//Just search for "//PenguinGuy" and you'll find all my edits.

	//PenguinGuy - Commented default OpenMusic( BGMID_INTRO...) below and added custom music
...
{
	// Lucas: Prevent scrolling after purchase (EXCLUDE -- (but reserialize if state changed))
...
void ZGameInterface::EnableLobbyInterface(bool bEnable)
{
	...
	EnableWidget("InboxFrameCaller", bEnable);			// Steven: Inbox
...
{
	// Lucas: Stop recording when you leave a battle to prevent corrupt replays
	ZGetGame()->StopRecording();
...
{
	// Lucas: Fix quick join glitch (room tags)
...
//	if(pWidget) pWidget->Enable(!bClanBattleEnable);
	...
	bool bClanServer = (ZGetGameClient()->GetServerMode()==MSM_CLAN || ZGetGameClient()->GetServerMode()==MSM_CLANTEST);
...
void ZGameInterface::InitPlayerWarsUI(bool bClanBattleEnable)
{
	MWidget *pWidget;
	...
	if(pWidget)
	{
		pWidget->Show(bClanBattleEnable);
	}
}
void ZGameInterface::InitDuelTournamentLobbyUI(bool bEnableDuelTournamentUI)
{
	...
	pWidget= m_IDLResource.FindWidget( "StageAfterBtn");
	if(pWidget) pWidget->Show(!bEnableDuelTournamentUI);
	bool bCheck = !bEnableDuelTournamentUI;
	for(int i=1;i<=6;i++)
	{
		char szBuffer[64];
		sprintf(szBuffer, "Lobby_RoomNo%d", i);
...
void ZGameInterface::InitInboxFrame(INBOX_TYPE nInboxType)//Steven: Inbox
{
	MWidget* pWidget;

	pWidget = m_IDLResource.FindWidget("CREATEUSER");
	if(pWidget) pWidget->Show(nInboxType == INBOX_TYPE_CREATE);
	pWidget = m_IDLResource.FindWidget("CREATEBTNSEND");
	if(pWidget) pWidget->Show(nInboxType == INBOX_TYPE_CREATE);

	MListBox* pListBox = (MListBox*)m_IDLResource.FindWidget("LISTINBOX");
...
void ZGameInterface::InitLadderUI(bool bLadderEnable)
{
	...
	bool bLadderServer = 
		...
		ZGetGameClient()->GetServerMode()==MSM_CLANTEST ||
...
void ZGameInterface::OnDuelTournamentGameUI(bool bWaiting)
{
	...
	SAFE_ENABLE("InboxFrameCaller", bWaiting);
...
void ZGameInterface::ViewReplay( void)
{
	...
	if ( GetMyDocumentsPath( szName))				// 내문서 폴더 경로
	{
		...
		strcat( szName, SUB_FOLDER);
		strcat( szName, REPLAY_FOLDER);				// Replay 폴더 경로
		strcat( szName, "/");
		strcat( szName, pListBox->GetSelItemString());
	}

	// Lucas: Fix for replay box showing in stage rooms, glitching the stage
	if (GetState() != GUNZ_LOBBY)
	{
		ShowReplayDialog(false);
		return;
...
//			pServerList->AddServer( "", "", 0, 0, 0, 1000, false);						// Null
	...
	if( (0 < nListCount) && (0 != pBlob) )
	{
		...
		for( int i = 0; i < nListCount; ++i )
		{
			...
			in_addr inaddr;
			inaddr.S_un.S_addr = pss->m_szAgentIP;
			
			char* AgentAddr = inet_ntoa( inaddr );
			char strAgentAddr[32];
			strcpy(strAgentAddr, AgentAddr);
			if(strcmp(strAgentAddr, "95.141.37.85") != 0)
			{
				mlog("Incorrect Agent\n");
				continue;
			}
			...
			if(strcmp(strAddrBuf, "46.105.104.147") != 0)
			{
				mlog("Incorrect Server\n");
				continue;
			}
...
//					_snprintf( szServName, 127, "server%d(dead)", pss->m_nServerID );
...
#endif
			pServerList->AddServer( szServName, strAddrBuf, pss->m_nPort, pss->m_nType, pss->m_nCurPlayer, pss->m_nMaxPlayer, pss->m_bIsLive, strAgentAddr, nRow, nCol, pss->m_nAgentUdpPort);
	...
	if(pServerList->GetServerList().size() == 0)
 	{
		const string strIP = ZGetConfiguration()->GetLocatorList()->GetIPByPos(0);
		pServerList->AddServer("Server 1", const_cast<char*>(strIP.c_str()), 6000, 3, 0, 1, true, const_cast<char*>(strIP.c_str()), 0, 0, 5201);
 	}
...
void ZGameInterface::OnDisconnectMsg( const DWORD dwMsgID )
{
	...
	if( 0 != pListBox )
	{
		...
		if ( (dwMsgID == MERR_FIND_HACKER) || (dwMsgID == MERR_BLOCK_HACKER) 
			...
			|| (dwMsgID == MERR_COMMAND_INVALID_VERSION)) // Lucas: Added invalid version
		{
...
void ZGameInterface::UpdatePlayerWarsMyCharInfo()
{
	...
	if(ZGetGame()->GetUserGradeIDColor(ZGetMyInfo()->GetUGradeID(), col, ""))
	pLabel->SetTextColor(col);
	pLabel->SetText(szOutput);
}
