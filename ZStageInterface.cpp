...
void ZStageInterface::OnCreate( void)
{
	...
	if ( pWidget)
		pWidget->Show( false);
	//Steven: Inbox
	pWidget = (MWidget*)pResource->FindWidget( "InboxFrame");
	if ( pWidget)
		pWidget->Show( false);
...
*/
	...
	if ( (ZGetGameClient()->AmIStageMaster() == true) && ( ZGetGameClient()->IsForcedEntry()))
	{
		if ( ZGetGameClient()->GetMatchStageSetting()->GetStageState() == STAGE_STATE_STANDBY)
		{
			...
			ZApplication::GetGameInterface()->EnableWidget( "Antilead", true);
		}
		else	// 마스터인데 다른사람들 게임중이면 인터페이스 Disable
		{
			...
			ZApplication::GetGameInterface()->EnableWidget( "Antilead", false);
...
void ZStageInterface::ChangeStageGameSetting( const MSTAGE_SETTING_NODE* pSetting)
{
	...
	if(pSetting->nGameType == MMATCH_GAMETYPE_DEATHMATCH_TEAM_TRIPLE)	
	{
	if(msg == 0)
	{
	msg= 1;
	ZChatOutput("Welcome to TripleTDM", ZChat::CMT_SYSTEM);
	ZChatOutput("To become yellow team, just double-click Red or Blue team button", ZChat::CMT_SYSTEM);
	ZChatOutput("for the Dead players, can watch the members in all teams at Observer mode", ZChat::CMT_SYSTEM);
	ZChatOutput("Start position : Red or Blue teams are normal (same as TDM). Yellow team middle of map (only official maps).", ZChat::CMT_SYSTEM);
	ZChatOutput("If all (three) teams are joined, then Start round.", ZChat::CMT_SYSTEM);
	}
	}
	...
	if ( (pSetting->nGameType == MMATCH_GAMETYPE_DEATHMATCH_SOLO) ||			// 데쓰매치 개인전이거나...
		 (pSetting->nGameType == MMATCH_GAMETYPE_KOTH) ||	
		 ...
		 (pSetting->nGameType == MMATCH_GAMETYPE_DUEL) ||					// 트레이닝이거나...
		 (pSetting->nGameType == MMATCH_GAMETYPE_BOUNTY))							// 듀얼모드 이면...
	{
		// 맵 이름 배경 이미지 변환
		if ( pAniMapImg)
		...
		(pSetting->nGameType == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC) ||
		 (pSetting->nGameType == MMATCH_GAMETYPE_GLADIATOR_TEAM) ||				// 칼전 팀전이거나...
		  (pSetting->nGameType == MMATCH_GAMETYPE_CTF) ||	
		  (pSetting->nGameType == MMATCH_GAMETYPE_DEATHMATCH_TEAM_TRIPLE) ||
		// (pSetting->nGameType == MMATCH_GAMETYPE_DEATHMATCH_TRIPPLE_TEAM) ||	
...
#ifdef Zombies
	else if ( pSetting->nGameType == MMATCH_GAMETYPE_QUEST || pSetting->nGameType == MMATCH_GAMETYPE_ZOMBIE)						// 퀘스트 모드이면...
#else
...
#ifdef Zombies
	if ( (pSetting->nGameType == MMATCH_GAMETYPE_SURVIVAL) || (pSetting->nGameType == MMATCH_GAMETYPE_QUEST) || pSetting->nGameType == MMATCH_GAMETYPE_ZOMBIE)
#else
...
//	ZApplication::GetGameInterface()->ShowWidget( "StageRoundCount", bShowRound);
	...
	if ( pWidget)
	{
		if ((pSetting->nGameType == MMATCH_GAMETYPE_DEATHMATCH_SOLO) ||
			...
			(pSetting->nGameType == MMATCH_GAMETYPE_BOUNTY))
...
#ifdef Zombies
	if ( (m_nGameType == MMATCH_GAMETYPE_SURVIVAL) || (m_nGameType == MMATCH_GAMETYPE_QUEST) || (m_nGameType == MMATCH_GAMETYPE_ZOMBIE))
#else
...
//		ZApplication::GetGameInterface()->EnableWidget( "StageMaxPlayer", false);
	...
	else
	{
		if ( ZGetGameClient()->AmIStageMaster())
		{
			...
			ZApplication::GetGameInterface()->EnableWidget( "Antilead", !bReady);
		}
		else
		{
