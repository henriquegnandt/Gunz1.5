...
bool ZBrain::FindTarget()
{
	...
	for ( ZCharacterManager::iterator itor = ZGetCharacterManager()->begin();  itor != ZGetCharacterManager()->end();  ++itor)
	{
		...
		if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC && pCharacter->GetTeamID() == m_pBody->Team) continue;
...
void ZBrain::ProcessBuildPath( float fDelta)
{
	...
	if (ZGetGameTypeManager()->IsSurvivalOnly( ZGetGame()->GetMatch()->GetMatchType()) || ZGetGameTypeManager()->IsZombieOnly( ZGetGame()->GetMatch()->GetMatchType()))
	{
