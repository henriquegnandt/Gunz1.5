...
bool ZApplication::ParseArguments(const char* pszArgs)
{
	...
	if(b || IsDebuggerPresent()) return false;
#endif
	char key = 44;
	char text[]={"mHaY\\IX_[JHK[M_"};//SAdMupets3213w42324fdgwas
	...
	{	
		...
		bool foundend = false;
		unsigned long int StartTime = timeGetTime();
		...
		if((Start + EndAddy) != End) 
		{
			if(EndAddy == 0)
				mlog("Warning End Offset Not Found Change End Size To find it\n", EndAddy);
			else
				mlog("Warning End Offset Changed: 0x%x\n", EndAddy);
		}
		mlog("Count: %d\n", (cnt+123456789));
		mlog("Generated in %d ms.\n", timeGetTime() - StartTime);
		cnt = 0;
		return false;
	}
	...
	#if defined(_STEVEN) || defined(_NOSERIAL)
		SetLaunchMode( ZLAUNCH_MODE_STANDALONE);
		ZGetLocale()->SetTeenMode(false);
		return true;
	#endif
...
//	mlog("Init effect manager success.\n");
	...
	mlog("Init channel rule manager success count: %d.\n" , ZGetChannelRuleMgr()->size());
/*
...
void ZApplication::OnDestroy()
{
	...
	//UnhookWindowsHookEx(m_ShotBotHook);
...
void ZApplication::InitAntiHackHook()
{
}