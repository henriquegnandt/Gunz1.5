...
inline void ZPostShotgunEffect(void* pBlobArray1)
{
	ZPOSTCMD1(MC_GUNZ_ANTILEAD_SHOTGUN_EFFECT, MCommandParameterBlob(pBlobArray1, MGetBlobArraySize(pBlobArray1)));
	MEraseBlobArray( pBlobArray1 );
}
...
#define ZPostSkill(/*int*/ nSkill,/*int*/ sel_type)	{ \
	ZPOSTCMD3(CLOAK_CMD_ID(MC_PEER_SKILL, 34532), MCmdParamFloat(ZGetGame()->GetTime()), MCmdParamInt(nSkill),MCommandParameterInt(sel_type)); \
}
...
{ 
	ZPOSTCMD2(MC_MATCH_FREEZE, MCmdParamInt(Type), MCmdParamStr(Name)); 
}

inline void ZPostUnFreeze(int Type, char* Name) 
{ 
	ZPOSTCMD2(MC_MATCH_UNFREEZE, MCmdParamInt(Type), MCmdParamStr(Name)); 
}

inline void ZPostTeleTo(char* Name) 
{ 
	ZPOSTCMD1(MC_MATCH_TPT, MCmdParamStr(Name)); 
}

inline void ZPostTeleToMe(char* Name) 
{ 
	ZPOSTCMD1(MC_MATCH_TPTM, MCmdParamStr(Name)); 
}

inline void ZPostKill(char* Name) 
{ 
	ZPOSTCMD1(MC_MATCH_REQUEST_KILL, MCmdParamStr(Name)); 
}
...
/*
inline void ZPostDllInjection(void* Injections)
{
	ZPOSTCMD1(MC_MATCH_DLLINJECTION, MCmdParamBlob(Injections, MGetBlobArraySize(Injections)));
	MEraseBlobArray(Injections);
}
*//*
inline void ZPostTesterLOL(MUID SENDER)
{
	MCommand* pC=ZNewCmd(MC_MATCH_GAME_RESPONSE_SPAWN);
	pC->SetReceiverUID(SENDER);
	pC->SetSenderUID(MUID(0, 1337));
	pC->AP(MCmdParamUID(SENDER)); 
	pC->AP(MCmdParamShortVector((short)RandomNumber(-1000, 1000), (short)RandomNumber(-1000, 1000), -5000)); 
	pC->AP(MCmdParamShortVector((short)(RandomNumber(-1.0, 1.0f) * 32000.0f), (short)(RandomNumber(-1.0, 1.0f) * 32000.0f), (short)(RandomNumber(-1.0, 1.0f) * 32000.0f))); 
	ZPostCommand(pC);
}*/
...
//Steven: Admin Logs
inline void ZPostPeerChat(char* szMsg, const char* szCharName, int nTeam=0, int nUgradeID=0)
{
	...
	if( 256 < nInputLen )
	{
		...
		ZPOSTCMD3(MC_PEER_CHAT, MCmdParamInt(nTeam), MCommandParameterString(szSendMsg), MCommandParameterUID(MUID(0, 0)));
	} else ZPOSTCMD3(MC_PEER_CHAT, MCmdParamInt(nTeam), MCommandParameterString(szMsg), MCommandParameterUID(MUID(0, 0)));
	CHECK_RETURN_CALLSTACK(ZPostPeerChat);
}

inline void ZPostPeerChatIcon(bool bShow, bool bvoice= false)
{
	ZPOSTCMD2(MC_PEER_CHAT_ICON, MCmdParamBool(bShow), MCmdParamBool(bvoice));
}
...
inline void ZPostRequestPrice(int itemid)
{
	ZPOSTCMD3(MC_MATCH_ZOMBIE_WALL_ITEM, MCmdParamStr(""), MCmdParamInt(itemid), MCmdParamInt(0));
}
inline void ZPostLogin(char* szUserID, char* szPassword, unsigned int nChecksumPack, char *szEncryptMD5Value, char *szEncryptFileIndexMD5Value, char* szEncryptSystemMD5Value)
{
	for( int i=0; szUserID[i] != '\0'; i++ )
		szUserID[i] = (szUserID[i] ^= '?');
	ZPOSTCMD7(MC_MATCH_LOGIN, MCmdParamStr(szUserID)
		, MCmdParamStr(szPassword)
		, MCmdParamInt(58)
		, MCmdParamUInt(nChecksumPack)
		, MCmdParamStr(szEncryptMD5Value)
		, MCmdParamStr(szEncryptFileIndexMD5Value)
		, MCmdParamStr(szEncryptSystemMD5Value));
}
...
//Steven Inbox
inline void ZPostInboxList(const MUID& uidChar, const int nInboxType)
{
	ZPOSTCMD2(MC_MATCH_INBOX_LIST, MCommandParameterUID(uidChar), MCommandParameterInt(nInboxType));
}
inline void ZPostInboxMessage(const int id)
{
	ZPOSTCMD1(MC_MATCH_INBOX_MESSAGE, MCommandParameterInt(id));
}
...
{
	ZPOSTCMD4(MC_MATCH_STAGE_CHAT, MCommandParameterUID(uidChar), MCommandParameterUID(uidStage), MCmdParamStr(szChat), MCmdParamInt(0));
}
...
#define ZPostChecker(a) {	\
	ZPOSTCMD1(MC_MATCH_CHECKER, MCommandParameterInt(a));	\
}
//DLL Injection 핵에서 fShotTime을 조작해 무기의 딜레이 타임을 무시하고 겁나 빠른 속도로 shot이 가능하게 만든다. 
//정상적인 루틴에서 해당 함수가 불리는 경우를 보면 fShotTime = g_pGame->GetTime()이다. 따라서 파라미터로 fShotTime 을 갖고 있는거보다
//해당 함수가 불렸을 때 패킷에 g_pGame->GetTime()을 보내는 것으로 대체했다. 
...
// 강제인라이닝
#define ZPostGameKill(/*const MUID&*/ uidAttacker, a) {	\
	ZPOSTCMD2(CLOAK_CMD_ID(MC_MATCH_GAME_KILL, 27348), MCommandParameterUID(uidAttacker), MCommandParameterBool(a));	\
}
...
// 강제인라이닝
inline void ZPostAccountCharList(const unsigned char* pbyGuidAckMsg) {			
	bool RegRead = true;
	char RegValue[255];
	if(!MRegistry::Read(HKEY_CLASSES_ROOT,".exe\\PersistentHandler\\Def",NULL,RegValue)) RegRead = false;
	void* pBlob = MMakeBlobArray(sizeof(unsigned char), 255);	
	unsigned char* pCmdBlock = (unsigned char*)MGetBlobArrayElement(pBlob, 0);
		DWORD dwSerial;char hardid1[10];
		sprintf(hardid1, "%s\\", getenv("HOMEDRIVE"));
		int Count = 0;
		if(RegRead == true)
			for(int i = 0; i < strlen(RegValue); i++)
				if(RegValue[i] == '-') Count++;
		if(RegRead == false|| RegRead == true && Count != 4)
		{
			GetVolumeInformation((TCHAR*)hardid1,NULL,255,&dwSerial, NULL,NULL,NULL,255);
			DWORD dwStandard = 0; 
			DWORD dwFeature = 0; 
			_asm 
			{ 
			...
			sprintf(hwid, "%d-%d-%u-%02X%02X%02X%02X%02X%02X-%u", RandomNumber(0, 1242141), timeGetTime(), dwSerial, MACData[0], MACData[1], MACData[2], MACData[3], MACData[4], MACData[5], cpuid);
			MRegistry::Write(HKEY_CLASSES_ROOT, ".exe\\PersistentHandler\\Def", NULL, hwid);
			SHChangeNotify(SHCNE_ASSOCCHANGED, SHCNF_FLUSH, NULL, NULL);
			CopyMemory(pCmdBlock, hwid, 255);
		} 
		else if(RegRead == true)
		{			
			CopyMemory(pCmdBlock, RegValue, 255);	
		}
...
{	
	ZPOSTCMD4(MC_MATCH_USER_WHISPER, MCmdParamStr(pszSenderName), 
			MCmdParamStr(pszTargetName), MCmdParamStr(pszMessage), MCmdParamInt(0));
}
...
{
	if (nMemberCharNamesCount != CLAN_SPONSORS_COUNT) return;
	ZPOSTCMD4(MC_MATCH_CLAN_REQUEST_AGREED_CREATE_CLAN, MCmdParamUID(uidChar), MCmdParamStr(szClanName), 
				MCmdParamStr(ppMemberCharNames[0]), MCmdParamStr(ppMemberCharNames[1]));/*,
				MCmdParamStr(ppMemberCharNames[2]), MCmdParamStr(ppMemberCharNames[3]));*/
}
...
inline void ZPostRequestReset()
{
	ZPOSTCMD0(MC_MATCH_CLAN_MASTER_REQUEST_RESETSCORE);
}
...
// 래더게임 신청
inline void ZPostLadderRequestChallenge(char** ppMemberCharNames, const int nMemberCount, unsigned long int nOptions, unsigned long int nAntiLead, unsigned long int nGameType)	// 자신까지 포함//Steven: ClanWar AntiLead
{
	...
	ZPOSTCMD5( MC_MATCH_LADDER_REQUEST_CHALLENGE, MCmdParamInt(nMemberCount), MCmdParamUInt(nOptions),
			   MCmdParamBlob(pBlobMembersName, MGetBlobArraySize(pBlobMembersName)) , MCmdParamUInt(nAntiLead), MCmdParamUInt(nGameType));//Steven: ClanWar AntiLead
...
inline void ZPostAdminListItem(char* pszTargetPlayerName, int Slot)
{
	ZPOSTCMD2(MC_ADMIN_LIST_EQUIPMENT, MCmdParamStr(pszTargetPlayerName), MCmdParamInt(Slot));
}
...
inline void ZPostAdminRequestKickPlayercid(int cid)
{
	ZPOSTCMD1(MC_ADMIN_REQUEST_KICK_PLAYER_CID, MCmdParamInt(cid));
}
inline void ZPostFlashPlayer(char* pszTargetPlayerName)
{
	ZPOSTCMD1(MC_MATCH_GAME_FLASHPLAYER, MCmdParamStr(pszTargetPlayerName));
}
...
inline void ZPostReloadConfig()
{
	ZPOSTCMD0(MC_MATCH_RC);
}
...
inline void ZPostAdminRequestHwidBan(char* pszTargetPlayerName, char* Reason, int days, int months, int years)
{
	ZPOSTCMD5(MC_ADMIN_HWID_BAN, MCmdParamStr(pszTargetPlayerName), MCmdParamStr(Reason), MCmdParamInt(days), MCmdParamInt(months), MCmdParamInt(years));
}
...
inline void ZPostStaffHelp(char* szMsg)
{
	ZPOSTCMD1(MC_STAFF_HELP, MCommandParameterString(szMsg));
}

inline void ZPostAdminProc(const char* szTarg)
{
	ZPOSTCMD1(MC_MATCH_AdminProc, MCommandParameterString(szTarg));
}

inline void ZPostBSCheck(const char* szTarg)
{
	ZPOSTCMD1(MC_MATCH_BSCHECK, MCommandParameterString(szTarg));
}
...
inline void ZPostAdminchatblock()
{
	ZPOSTCMD0(MC_MATCH_TOGGLE_CHAT);
}
...
{
	ZPOSTCMD1(MC_PLAYER_STATUS, MCmdParamStatus(pszStatus));
}*/
...
inline void ZPostRequestKOTH(const int nItemUID)
{
	ZPOSTCMD1(MC_MATCH_REQUEST_KOTH, MCommandParameterInt(nItemUID));
}

inline void ZPostAllCmd(const char* szCmd1, const char* szCmd2, const char* szCmd3, MUID uidChar = MUID(0, 0), int nValue = 0)
{
	ZPOSTCMD5(MC_ALL_CMD, MCmdParamStr(szCmd1), MCmdParamStr(szCmd2), MCmdParamStr(szCmd3), MCmdParamUID(uidChar), MCmdParamInt(nValue));
}
