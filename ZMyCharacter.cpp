...
void ZMyCharacter::OnShotRange()
{
	...
	if( ( wtype == MWT_SHOTGUN ) || ( wtype == MWT_SAWED_SHOTGUN ) || (wtype == MWT_DOUBLE_SHOTGUN) )
	{
		sel_type = GetItems()->GetSelectedWeaponParts();
	...
	else
	{
		...
		sel_type = GetItems()->GetSelectedWeaponParts();
		vTo = vWeaponPos+10000.f*r;
	}

	if(ZGetGameClient()->GetMatchStageSetting()->IsQuestDrived() == false && ZGetGameClient()->GetMatchStageSetting()->IsAntiLead() == true || ZGetGameClient()->GetChannelType() == MCHANNEL_TYPE_DUELTOURNAMENT && strstr(ZGetGameClient()->GetChannelName(), "(AntiLead)"))
	{	
		float fShotTime = ZGetGame()->GetTime();
		if(!IsVisible()) return;
		OnShot();
		fShotTime-=m_fTimeOffset;
		ZItem *pItem = GetItems()->GetItem(sel_type);
		if(!pItem || !pItem->GetDesc()) return;
		if (CheckValidShotTime(pItem->GetDescID(), fShotTime, pItem)) 
		{
			UpdateValidShotTime(pItem->GetDescID(), fShotTime);
		} 
		else 
		{
			return;
		...
		{
			ZGetGame()->OnPeerShot_Shotgun(sel_type,this,fShotTime,vWeaponPos,vTo);
			return;
		} else {
			ZGetGame()->OnPeerShot_Range(sel_type,this->GetUID(),fShotTime,vWeaponPos,vTo);
			rvector position;
			GetWeaponTypePos( weapon_dummy_muzzle_flash , &position );
...
void ZMyCharacter::OnShotItem()
{
	...
	// 바닥에 떨구는 아이템인 경우
		...
		nWeaponType==MWT_FOOD || nWeaponType==MWT_CLAYMORE )
	{
		...
		if(nWeaponType==MWT_CLAYMORE) 
		{
			ZPostShotSp(pos,velocity,ZC_WEAPON_SP_CLAYMORE,sel_type);
		} else {
...
void ZMyCharacter::OnGadget_Snifer(float zoom)
{
	ZMyCharaterStatusBitPacking & zStatus = m_statusFlags.Ref();
...
void ZMyCharacter::ProcessGuard()
{
	...
	if(type!=MWT_KATANA && type!=MWT_DOUBLE_KATANA && type != MWT_GREAT_SWORD || type == MWT_DAGGER || type == MWT_DUAL_DAGGER) 
	{
		CHECK_RETURN_CALLSTACK(ProcessGuard);
		return; 
	}
...
void ZMyCharacter::ProcessShot()
{
	...
	//if(MEvent::GetRButtonState()) return; // 전코드: if (MEvent::GetRButtonState()) return;  
...
//	if(GetItems()->GetSelectedWeapon()->IsEmpty() == true) return;		// 총알부족
	...
	*/
	...
	switch( m_pVMesh->m_SelectWeaponMotionType ) {
		...
		default:				
			if(ZGetGame()->m_pMyCharacter->GetStatus().Ref().bIsRandom == true && !ZGetGameInterface()->GetCombatInterface()->GetObserverMode())
			{
				ZGetGame()->m_pMyCharacter->GetStatus().CheckCrc();
				ZGetGame()->m_pMyCharacter->GetStatus().Ref().bIsRandom = false;
				unsigned long int nowtime = timeGetTime();
				if((nowtime - ZGetGame()->m_pMyCharacter->GetStatus().Ref().TimeActivated) <= 150)
				{
					ZGetGame()->m_pMyCharacter->GetStatus().Ref().nBSCount++;
					ZGetGame()->m_pMyCharacter->GetStatus().Ref().LastWarning = nowtime;
				}
				if((nowtime - ZGetGame()->m_pMyCharacter->GetStatus().Ref().LastWarning) >= 1200000)
				{
					ZGetGame()->m_pMyCharacter->GetStatus().Ref().nBSCount = 0;
				}
				if(ZGetGame()->m_pMyCharacter->GetStatus().Ref().nBSCount >= 4)
				{
					char szText[340];
					sprintf(szText, "SB Red Cursor Event Alert");
...
void ZMyCharacter::OnDashAttacked(rvector &dir)
{
	...
	if(uStatus.m_bBlast || uStatus.m_bBlastDrop || uStatus.m_bBlastStand )
		return;
	bool bForDebugRegister = (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2 || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC);
...
void ZMyCharacter::OnBlast(rvector &dir, MUID ATTACKER)
{
	//jintriple3 메모리 프록시...비트 패킹..
	ZCharaterStatusBitPacking & uStatus = m_dwStatusBitPackingValue.Ref();
	ZMyCharaterStatusBitPacking & zStatus = m_statusFlags.Ref();
	...
	if(uStatus.m_bBlast || uStatus.m_bBlastDrop || uStatus.m_bBlastStand )
		return;
	bool bForDebugRegister = ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2 || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC || ZGetGame()->GetMatch()->GetMatchType() ==MMATCH_GAMETYPE_CTF;
	if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2 || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2_NPC || ZGetGame()->GetMatch()->GetMatchType() ==MMATCH_GAMETYPE_CTF) 
		...
		//ZGetGame()->PeersLifeSaver->Add(ATTACKER, Flip);
	#endif
	// 속도를 없애는 것들은.. false 해줘야 뜬다.
	zStatus.m_bSkill=false;
	uStatus.m_bStun=false;
...
//	if( UsingStamina( ZUS_Tumble )==false )	return;
		...
		GetItems()->GetSelectedWeapon()->GetDesc()->m_nType.Ref()==MMIT_MELEE) 
	{
		...
		if (!zStatus.m_bWallHang && !zStatus.m_bSkill && !zStatus.m_bShot && !zStatus.m_bShotReturn)
			if (ZGetMyInfo()->IsAdminGrade() || ZGetMyInfo()->IsVipGrade())
				ZPostDash(vPos, vDir, sel_type, ZGetConfiguration()->GetEtc()->nDashColor);
			else
				ZPostDash(vPos, vDir, sel_type, 0);
...
void ZMyCharacter::InitItemBullet(bool onlynades)
{
	ZCharacter::InitItemBullet(onlynades);
...
//		if(m_bShot) 
		...
		break;
	case ZDW_UPPERCUT :
		// if(strstr(ZGetGameClient()->GetStageName(),"[NF]")) return;
		if(zStatus.m_bSkill) {	// 아직 상태를 벗어나지 않았다면,,
			...
			if(type == MWT_KATANA || type == MWT_DOUBLE_KATANA || MWT_GREAT_SWORD ) {
				ZPostSkill( /*g_pGame->GetTime(),*/ ZC_SKILL_UPPERCUT , sel_type );
			}
		}
		break;
...
{
	//if(strstr(ZGetGameClient()->GetStageName(),"[NM]")) return;
	ZMyCharaterStatusBitPacking & zStatus = m_statusFlags.Ref();
...
float ZMyCharacter::GetGravityConst()	// 중력의 영향을 얼만큼 받는지..
{
	...
	if(zStatus.m_bSlash)
	{
		...
		// Lucas: Bug fix (pDesc NULL check)
...
void ZMyCharacter::OnDamaged(ZObject* pAttacker, rvector srcPos, ZDAMAGETYPE damageType, MMatchWeaponType weaponType, float fDamage, float fPiercingRatio, int nMeleeType)
{
	/*if (bEventReduce)
	{
	 ...
	 if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_GUNGAME && damageType==ZD_MELEE && pAttacker->GetUID() != ZGetGame()->m_pMyCharacter->GetUID())
	 {
	  SetHP(0);
	  SetAP(0);
	  ZPostDie(pAttacker->GetUID());
	  ZPostGameKill(pAttacker->GetUID(), true);
	  return;
	 }
